...
old_projection_start = """      const view = (req.query.view as string) || 'yearly';
      if (!['weekly', 'monthly', 'yearly'].includes(view)) {
        return res.status(400).json({ error: 'view must be weekly, monthly, or yearly' });
      }

      let marketFactors"""
new_projection_start = """      const view = (req.query.view as string) || 'yearly';
      if (!['weekly', 'monthly', 'yearly'].includes(view)) {
        return res.status(400).json({ error: 'view must be weekly, monthly, or yearly' });
      }
      ...
      let marketFactors"""
content = content.replace(old_projection_start, new_projection_start)

# 3. Replace digital score block
old_digital = """          try {
...
# 4. Replace data source signals block in projection
old_ds_projection = """          let propId: string | undefined;
...
content = content.replace(old_ds_projection, new_ds_projection)

# 5. Replace data source signals block in /data-sources/:dealId
