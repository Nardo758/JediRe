...
const CAT_I: LeasingCategoryDef = {
  ...
  fields: [
    ...
    {
      id: 'i_other_income_per_unit',
      label: 'Other Income ($/unit/mo)',
      path: 'proforma.other_income_per_unit',
      type: 'currency',
      tier: 'advanced',
      modes: 'all',
      platformDefault: 50,
      min: 0,
      tooltip: 'Ancillary other income per unit per month (parking, pet fees, laundry, RUBS, valet trash, etc.). Level driver — set the base amount here; use the growth % row below to escalate it over the hold period.',
      defaultSource: 'Platform $50/unit/mo conservative anchor',
      category: 'I',
    },
    {
