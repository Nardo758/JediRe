...
const DealsTab: React.FC<DealsTabProps> = ({ marketId, summary, onUpdate }) => {
  ...
  const featuredDeal = {
    ...
    ownerEntity: 'Peachtree Capital Partners LLC',
  };
  ...
  const dealActivityRows = [
    { property: 'Parkside at Buckhead', type: 'Listed', units: 280, price: '$52.4M', perUnit: '$187K', assessment: '\u26A0\uFE0F S-05 cluster risk \u2014 3 deliveries within 0.5mi by Q3 2026', pcsRank: 5, pcsMovement: 'down' as Movement, pcsMovementDelta: 2, quadrant: 'Hype Risk' as Quadrant, targetScore: 67, owner: 'Greystar RE Partners' },
    { property: 'The Vue at Midtown', type: 'Listed', units: 196, price: '$38.2M', perUnit: '$195K', assessment: '\u2705 Strong fundamentals \u2014 D-09: 84, low supply, T-01 validated', pcsRank: 2, pcsMovement: 'up' as Movement, pcsMovementDelta: 1, quadrant: 'Validated Winner' as Quadrant, targetScore: 88, owner: 'Cortland Partners' },
    { property: 'Glenwood Gardens', type: 'Closed', units: 320, price: '$64.0M', perUnit: '$200K', assessment: '\u2705 Fair price \u2014 within 3% of AI estimate, good basis', pcsRank: 8, pcsMovement: 'neutral' as Movement, pcsMovementDelta: 0, quadrant: 'Hidden Gem' as Quadrant, targetScore: 79, owner: 'Waterton Associates' },
    { property: 'Cascade Pointe', type: 'Closed', units: 148, price: '$24.4M', perUnit: '$165K', assessment: '\u26A0\uFE0F Buyer overpaid by ~$12K/unit vs AI comp model', pcsRank: 22, pcsMovement: 'down' as Movement, pcsMovementDelta: 6, quadrant: 'Dead Weight' as Quadrant, targetScore: 41, owner: 'Local Co LLC' },
  ];
  ...
  const renderTargetScore = (score: number) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${color}`} title="Target Score: acquisition attractiveness based on performance gap, hold stress, and debt maturity">
        TS: {score}
      </span>
    );
  };
  ...
  const buildAiInsight = (): string => {
    ...
    parts.push(`PCS rank #${featuredDeal.pcsRank} but expected rank #${featuredDeal.performanceGap.expectedRank} \u2014 ${featuredDeal.performanceGap.gap}-position gap signals operational underperformance, not location weakness.`);
    parts.push(`Bottom-quartile management (${featuredDeal.managementPcsPercentile}th pctile) suggests significant operational upside with new operator.`);
    parts.push(`Triple trigger target \u2014 underperforming + Year ${Math.floor(featuredDeal.holdYears)} hold + est. maturity ${featuredDeal.debtMaturity}.`);
    parts.push(`Traffic qualified with T-07 trajectory at ${featuredDeal.trajectory} (${featuredDeal.trajectoryLabel}).`);
    return parts.join(' ');
  };
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 text-sm text-gray-500">
                        <span className="text-[11px]">Owner: <span className="font-semibold text-gray-700">{featuredDeal.ownerEntity}</span></span>
                      </div>
                    </div>
