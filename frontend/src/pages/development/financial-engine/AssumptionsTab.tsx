...
export function AssumptionsTab({ dealId, deal, dealType, assumptions, modelResults, onAssumptionsChange, onTabChange, onF9Refresh, onLvTreatmentViewChange, f9Financials, sourceDocuments }: FinancialEngineTabProps) {
  ...
  const allSections: Array<{ id: string; sec: number; label: string; rows: RowDef[]; sectionGroup: 'A' | 'B' }> = [
    { id: '5-traf', sec: 5, label: '5A  REVENUE › M07 TRAFFIC INTEL · LEASE VELOCITY INPUTS · CONCESSIONS  [Stabilization Path: 80%→95%]',
        ...
        <LeasingAssumptionsTab
          ...
          hasLatLng={
            deal != null && (
              ...
              (Number((deal as any).latitude) !== 0 && !isNaN(Number((deal as any).latitude)) &&
               Number((deal as any).longitude) !== 0 && !isNaN(Number((deal as any).longitude)))
            )
          }
