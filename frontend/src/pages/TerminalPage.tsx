...
const WIDGET_CATALOG = [
  ...
  {id:"neuralhub",  label:"Neural Network Hub",     desc:"Live agent runs, recent dispatches, platform events & ask-the-network",  category:"INTEL",  color:"#A78BFA"},
...
export default function TerminalPage() {
  ...
  const WidgetNeuralHub = () => {
    type RunRow = {
      id: string; agent_id: string; trigger_event: string;
      deal_id: string | null; user_id: string | null;
      started_at: string | null; completed_at: string | null;
      created_at: string; status?: string; error?: string | null;
    };
    type EventRow = {
    ...
    const statusColor = (s?: string) =>
      ...
      s === 'failed'    ? T.text.red   :
      s === 'running'   ? T.text.cyan  : T.text.muted;
      ...
      <div style={{flex:1, display:'flex', flexDirection:'column', overflow:'hidden', animation:'fadeIn 0.15s'}}>
        ...
        <div style={{flex:1, overflow:'auto', minHeight:0}}>
          {tab === 'live' && (
            ...
            ) : running.map(r => <RunLine key={r.id} r={{...r, status:'running'}} showStatus/>)
          )}
