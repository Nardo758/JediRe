...
export function AssetsTab() {
  ...
  const fetchFileCounts = useCallback(async () => {
    try {
      const res = await apiClient.get('/api/v1/data-library-assets/file-counts');
      setFileCounts(res.data.counts || {});
    } catch (err) {
  ...
  useEffect(() => { fetchFileCounts(); }, [fetchFileCounts]);
    ...
    <div>
      <div style={{ padding: '16px 20px', borderBottom: `1px solid ${BT.border.subtle}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 10, color: BT.text.muted, fontFamily: MONO }}>{total} ASSETS</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ flex: 1, padding: '6px 10px', background: BT.bg.input, border: `1px solid ${BT.border.medium}`, color: BT.text.primary, fontFamily: MONO, fontSize: 11 }}
          />
          <select value={filterType} onChange={e => setFilterType(e.target.value)}
            style={{ padding: '6px 10px', background: BT.bg.input, border: `1px solid ${BT.border.medium}`, color: BT.text.primary, fontFamily: MONO, fontSize: 11 }}
          >
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: MONO, fontSize: 10 }}>
          <thead>
            <tr style={{ background: BT.bg.panelAlt }}>
              {[
                ...
                <th key={col.key} onClick={() => col.key !== 'files' && handleSort(col.key)}
                  style={{
                    padding: '8px 6px', textAlign: 'left', color: BT.text.muted,
                    fontWeight: 600, cursor: col.key !== 'files' ? 'pointer' : 'default',
                    whiteSpace: 'nowrap', borderBottom: `1px solid ${BT.border.medium}`,
                    fontSize: 9, letterSpacing: 0.5,
                  }}
                >
                  {col.label}{col.key !== 'files' ? sortArrow(col.key) : ''}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={13} style={{ padding: 40, textAlign: 'center', color: BT.text.muted }}>Loading assets...</td></tr>
            ) : assets.length === 0 ? (
              <tr><td colSpan={13} style={{ padding: 40, textAlign: 'center', color: BT.text.muted }}>
              ...
              assets.map((a, i) => {
                ...
                const fCount = fileCounts[a.id] ?? 0;
                return (
                  <tr key={a.id} onClick={() => setEditingAsset(a)}
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const color = occPct != null && occPct >= 93 ? BT.text.green : occPct != null ? BT.text.orange : BT.text.muted;
                      return (<td style={{ padding: '6px', textAlign: 'right', color }}>{occPct != null ? `${occPct.toFixed(1)}%` : '\u2014'}</td>);
                    })()}
                    ...
                    <td style={{ padding: '6px' }}>
                      <span style={{ fontSize: 9, fontWeight: 600, color: src.color, padding: '1px 6px', border: `1px solid ${src.color}33`, background: `${src.color}11` }}>{src.label}</span>
                    </td>
                    <td style={{ padding: '6px', textAlign: 'center' }}>
                      {fCount > 0 ? (
                        <button onClick={e => { e.stopPropagation(); navigateToFiles(a.id); }}
                          style={{
                        ...
                        <span style={{ color: BT.text.muted, fontSize: 10 }}>0</span>
                      )}
                    </td>
                    <td style={{ padding: '6px', textAlign: 'center' }}>
                      <div style={{ width: 28, height: 4, background: BT.bg.panelAlt, position: 'relative', display: 'inline-block' }}>
                        <div style={{ width: `${Math.min(a.data_quality_score || 0, 100)}%`, height: '100%', background: (a.data_quality_score || 0) > 70 ? BT.text.green : (a.data_quality_score || 0) > 40 ? BT.text.amber : BT.text.red }} />
                      </div>
      ...
      {total > 50 && (
        <div style={{ padding: '10px 16px', borderTop: `1px solid ${BT.border.subtle}`, textAlign: 'center' }}>
          <span style={{ fontSize: 10, color: BT.text.muted, fontFamily: MONO }}>Showing {assets.length} of {total} assets</span>
        </div>
      )}
      ...
      {editingAsset && (
        <AssetDetailModal
          ...
          onSave={() => { fetchAssets(); fetchStats(); fetchFileCounts(); }}
        />
      )}
    </div>
  );
