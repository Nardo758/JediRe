...
import {
  RadarChart, Radar, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Tooltip as ReTooltip,
...
const PROPERTY_GATE_FIELDS = [
  { value: 'product_type',        label: 'Product Type' },
  { value: 'unit_count',          label: 'Unit Count' },
  { value: 'year_built',          label: 'Year Built' },
  { value: 'zoning_utilization',  label: 'Zoning Utilization %' },
  { value: 'str_regulations',     label: 'STR Regulations' },
...
const OPERATOR_LABELS: Record<string, string> = {
  gte: '≥', lte: '≤', eq: '=', in: 'in', not_in: 'not in', between: 'between',
};
...
const DEFAULT_LOCATION: LocationWeights = {
  transit: 0.5, schools: 0.5, employment: 0.5, retail: 0.5,
  parks: 0.5, healthcare: 0.5, restaurants: 0.5, entertainment: 0.5, radius_miles: 1.0,
};
...
function normalizeWeights(w: SignalWeights): SignalWeights {
  ...
  const factor = 1 / sum;
  return {
    demand:   parseFloat((w.demand   * factor).toFixed(4)),
    supply:   parseFloat((w.supply   * factor).toFixed(4)),
    momentum: parseFloat((w.momentum * factor).toFixed(4)),
    position: parseFloat((w.position * factor).toFixed(4)),
    risk:     parseFloat((w.risk     * factor).toFixed(4)),
  };
}
...
const btnSx = (variant: 'primary' | 'ghost' | 'danger' | 'amber'): React.CSSProperties => ({
  ...
  color:
    ...
    '#fff',
  border: variant === 'ghost' ? `1px solid ${border.subtle}` : 'none',
...
// ────────────────────────────────────────────────────────────
function SignalWeightSliders({ weights, onChange }: {
  weights: SignalWeights;
  onChange: (w: SignalWeights) => void;
}) {
  const chartData = SIGNAL_KEYS.map(k => ({ key: k, val: Math.round(weights[k] * 100) }));
  const sum = Object.values(weights).reduce((a, b) => a + b, 0);
  const sumOk = Math.abs(sum - 1.0) < 0.015;

  function handleSlider(k: keyof SignalWeights, rawVal: number) {
    const newW = { ...weights, [k]: rawVal / 100 };
    onChange(normalizeWeights(newW));
  }
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{
          ...
          <span style={{ fontFamily: font.mono, fontSize: fz.sm, color: sumOk ? text.green : text.red }}>
            {sumOk ? '✓ WEIGHTS SUM TO 100%' : `⚠ SUM = ${Math.round(sum * 100)}% — will auto-normalize`}
          </span>
          <button
            style={{ ...btnSx('ghost'), fontSize: fz.xs, padding: '2px 8px', marginLeft: 'auto' }}
        ...
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 4, left: 4 }}>
            ...
            <Bar dataKey="val" radius={[3, 3, 0, 0]}>
              {chartData.map(d => (
                <Cell key={d.key} fill={SIGNAL_COLORS[d.key as keyof SignalWeights]} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
...
// ────────────────────────────────────────────────────────────
function PropertyGateBuilder({ gates, onChange, title }: {
  gates: PropertyGate[];
  onChange: (g: PropertyGate[]) => void;
  title: string;
}) {
  function updateGate(id: string, patch: Partial<PropertyGate>) {
    onChange(gates.map(g => g.id === id ? { ...g, ...patch } : g));
  }
    ...
    <div>
      ...
      {gates.map(gate => (
        <div key={gate.id} style={{
          display: 'grid', gridTemplateColumns: '1fr auto 1fr auto auto auto', gap: 8,
          marginBottom: 8, padding: '10px', borderRadius: 4,
          background: bg.panelAlt, border: `1px solid ${border.subtle}`,
          alignItems: 'center',
        }}>
          <select
            value={gate.field}
            onChange={e => updateGate(gate.id, { field: e.target.value })}
            style={{ ...inputSx }}
          >
            {PROPERTY_GATE_FIELDS.map(f => (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <input
              type="checkbox" checked={gate.hard}
          ...
          {!gate.hard && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ fontFamily: font.mono, fontSize: fz.xs, color: text.amber }}>-</span>
              <input
                type="number" min={0} max={50} step={1}
                value={gate.penalty ?? 0}
          ...
          <button
            style={{ ...btnSx('danger'), padding: '5px 10px', fontSize: fz.xs }}
...
// ────────────────────────────────────────────────────────────
// LOCATION WEIGHTS TAB
// ────────────────────────────────────────────────────────────
function LocationWeightsTab({ weights, onChange }: {
  weights: LocationWeights;
  onChange: (w: LocationWeights) => void;
}) {
  return (
    <div>
        ...
        {LOCATION_AMENITY_KEYS.map(k => (
          <div key={k}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <span style={{ fontFamily: font.mono, fontSize: fz.base, color: text.amber }}>
                {Math.round(weights[k] * 10)}/10
              </span>
            </div>
            <input
...
// ────────────────────────────────────────────────────────────
function FinancialCriteriaTab({ criteria, onChange }: {
  criteria: FinancialCriteria;
  onChange: (c: FinancialCriteria) => void;
}) {
...
// ────────────────────────────────────────────────────────────
// EXECUTION PROFILE TAB
// ────────────────────────────────────────────────────────────
function ExecutionProfileTab({ profile, onChange }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontFamily: font.mono, fontSize: fz.sm, color: text.muted }}>MIN</span>
          <input
            type="range" min={1} max={20} step={1}
            value={profile.hold_period_min}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
          <span style={{ fontFamily: font.mono, fontSize: fz.sm, color: text.muted }}>MAX</span>
          <input
            type="range" min={1} max={30} step={1}
            value={profile.hold_period_max}
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {EXIT_TYPES.map(et => (
            <button
              key={et.value}
              onClick={() => onChange({ ...profile, exit_type: et.value })}
              style={{
                ...btnSx(profile.exit_type === et.value ? 'primary' : 'ghost'),
                fontSize: fz.sm,
              }}
            >
              {et.label}
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <input
            ...
            value={profile.stabilization_months}
...
function StrategyEditorModal({
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await onSave({ ...editor, signal_weights: normalizeWeights(editor.signal_weights) });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontFamily: font.mono, fontSize: fz.xs, color: text.muted, letterSpacing: '0.1em' }}>
        ...
        {/* META */}
        <div style={{ padding: '14px 20px', borderBottom: `1px solid ${border.subtle}`, display: 'flex', gap: 12 }}>
          <div style={{ flex: 2 }}>
        ...
        {/* TAB BAR */}
        <div style={{ display: 'flex', borderBottom: `1px solid ${border.subtle}`, background: bg.topBar }}>
          {TABS.map(t => (
            <button
              key={t.key}
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px' }}>
          ...
          {tab === 'gates' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              <PropertyGateBuilder
                title="PROPERTY GATES"
                gates={editor.property_gates}
                onChange={g => setEditor(s => ({ ...s, property_gates: g }))}
              />
              <div style={{ borderTop: `1px solid ${border.subtle}`, paddingTop: 20 }}>
                <PropertyGateBuilder
                  title="RISK GATES"
                  gates={editor.risk_gates}
          ...
          {tab === 'location' && (
            <LocationWeightsTab
              weights={editor.location_weights}
              onChange={w => setEditor(s => ({ ...s, location_weights: w }))}
            />
          )}
          {tab === 'financial' && (
            <FinancialCriteriaTab
              criteria={editor.financial_criteria}
              onChange={c => setEditor(s => ({ ...s, financial_criteria: c }))}
            />
          )}
          {tab === 'execution' && (
            <ExecutionProfileTab
              profile={editor.execution_profile}
        ...
        {/* FOOTER */}
        <div style={{
          padding: '14px 20px', borderTop: `1px solid ${border.subtle}`,
          display: 'flex', justifyContent: 'flex-end', gap: 10,
          background: bg.topBar,
        }}>
          <button style={btnSx('ghost')} onClick={onClose}>CANCEL</button>
          <button
            style={{ ...btnSx('primary'), opacity: saving ? 0.6 : 1 }}
            onClick={handleSave}
  ...
  return (
    <ResponsiveContainer width="100%" height={140}>
      <RadarChart cx="50%" cy="50%" outerRadius={50} data={data}>
        <PolarGrid stroke={border.subtle} />
        <PolarAngleAxis dataKey="subject" tick={{ fill: text.muted, fontSize: 8, fontFamily: font.mono }} />
        <PolarRadiusAxis angle={30} domain={[0, 100]} tick={false} axisLine={false} />
        <Radar
          name="weights" dataKey="value"
          stroke={text.purple} fill={text.purple} fillOpacity={0.25}
        />
        <ReTooltip
          contentStyle={{ background: bg.panel, border: `1px solid ${border.subtle}`, fontFamily: font.mono, fontSize: 10 }}
          labelStyle={{ color: text.primary }}
          itemStyle={{ color: text.purple }}
        />
      </RadarChart>
    </ResponsiveContainer>
  );
...
// STRATEGY CARD
// ────────────────────────────────────────────────────────────
function StrategyCard({ strategy, onEdit, onDelete, onClone }: {
  strategy: Strategy;
  onEdit: () => void;
  onDelete: () => void;
  onClone: () => void;
  ...
  return (
    <div
      style={{
        ...panelSx,
        border: `1px solid ${hovered ? border.medium : border.subtle}`,
        transition: 'border-color 0.15s, box-shadow 0.15s',
        boxShadow: hovered ? `0 4px 20px rgba(167,139,250,0.10)` : 'none',
        display: 'flex', flexDirection: 'column',
      }}
    ...
    >
      {/* Card header */}
      <div style={{
        padding: '12px 14px', borderBottom: `1px solid ${border.subtle}`,
      ...
      <div style={{ padding: '10px 14px' }}>
        {SIGNAL_KEYS.map(k => (
          <div key={k} style={{ marginBottom: 5 }}>
            ...
            <div style={{ height: 3, background: border.subtle, borderRadius: 2 }}>
              <div style={{
                height: '100%', borderRadius: 2,
                background: SIGNAL_COLORS[k],
                width: `${Math.round(weights[k] * 100)}%`,
      ...
      <div style={{ padding: '0 14px 10px', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <span style={{
          background: '#F5A62318', border: `1px solid #F5A62340`,
          color: text.amber, fontFamily: font.mono, fontSize: fz.xs, padding: '2px 7px', borderRadius: 3,
        }}>
          TOP: {topKey.toUpperCase()}
        </span>
        {gateCount > 0 && (
          <span style={{
        ...
        {strategy.execution_profile?.exit_type && (
          <span style={{
            ...
            {strategy.execution_profile.exit_type.replace('_', ' ').toUpperCase()}
          </span>
        )}
      </div>
      ...
      {/* Actions */}
      <div style={{
        padding: '10px 14px', borderTop: `1px solid ${border.subtle}`,
        display: 'flex', gap: 6, justifyContent: 'flex-end',
      }}>
        <button style={{ ...btnSx('ghost'), fontSize: fz.xs, padding: '4px 10px' }} onClick={onClone}>
          CLONE
        </button>
        {!strategy.is_system_template && (
          <>
            <button style={{ ...btnSx('ghost'), fontSize: fz.xs, padding: '4px 10px' }} onClick={onEdit}>
              EDIT
            </button>
            <button style={{ ...btnSx('danger'), fontSize: fz.xs, padding: '4px 10px' }} onClick={onDelete}>
              DELETE
            </button>
          </>
        )}
        {strategy.is_system_template && (
          <button style={{ ...btnSx('amber'), fontSize: fz.xs, padding: '4px 10px' }} onClick={onClone}>
            CLONE TO EDIT
          </button>
        )}
      </div>
...
// TEMPLATE GALLERY
// ────────────────────────────────────────────────────────────
function TemplateGallery({ templates, onClone }: {
  templates: Strategy[];
  ...
  return (
    <div>
      <div style={{
        ...
        paddingBottom: 10, borderBottom: `1px solid ${border.subtle}`,
      }}>
        <span style={{
          background: '#A78BFA20', border: `1px solid ${text.purple}50`,
          color: text.purple, fontFamily: font.mono, fontSize: fz.xs,
          padding: '2px 8px', borderRadius: 3,
        }}>SYSTEM TEMPLATES</span>
        <span style={{ fontFamily: font.mono, fontSize: fz.sm, color: text.muted }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
        {templates.map(t => (
          <div key={t.id} style={{ ...panelSx, border: `1px solid ${text.purple}30`, overflow: 'hidden' }}>
            <div style={{ padding: '12px 14px', borderBottom: `1px solid ${border.subtle}`, background: bg.header }}>
              <div style={{ fontFamily: font.mono, fontSize: fz.base, color: text.primary, fontWeight: 700 }}>
                {t.name}
              </div>
              {t.description && (
            ...
            <div style={{ padding: '10px 14px', borderTop: `1px solid ${border.subtle}`, display: 'flex', justifyContent: 'flex-end' }}>
              <button
                style={{ ...btnSx('amber'), fontSize: fz.xs, padding: '5px 12px' }}
...
export function M08StrategyBuilderPage() {
  const {
    ...
    fetchStrategies,
    createStrategy, updateStrategy, deleteStrategy, cloneStrategy,
  } = useStrategyStore();
  ...
  async function handleSave(state: EditorState) {
    const payload = {
      ...
      property_gates: state.property_gates,
      risk_gates: state.risk_gates,
  ...
  return (
    <div style={{ background: bg.terminal, minHeight: '100vh', color: text.primary }}>
      {/* TOP BAR */}
      <div style={{
        background: bg.topBar, borderBottom: `1px solid ${border.subtle}`,
        padding: '0 24px', display: 'flex', alignItems: 'center', gap: 16, height: 56,
      }}>
        <div style={{ fontFamily: font.mono, fontSize: fz.xs, color: text.muted, letterSpacing: '0.1em' }}>
          SETTINGS
        </div>
        <div style={{ color: border.medium, fontSize: 14 }}>›</div>
            ...
            <div style={{ marginBottom: 32 }}>
              <div style={{
                ...
                <span style={{
                  ...
                  fontFamily: font.mono, fontSize: fz.xs, padding: '2px 6px', borderRadius: 10,
                }}>
                  {filtered.length}
                </span>
              </div>
              ...
              {filtered.length === 0 ? (
                <div style={{
                  ...panelSx, padding: '40px',
                  textAlign: 'center', color: text.muted, fontFamily: font.mono, fontSize: fz.base,
                }}>
                  {search
                    ? 'No strategies match your search.'
                    : 'No strategies configured — create your first strategy or clone a system template below.'}
                </div>
              ) : (
      ...
      {cloneModalOpen && (
        <div style={{
          ...
          <div style={{ ...panelSx, width: 400, padding: 24, border: `1px solid ${border.medium}` }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button
                style={{ ...btnSx('amber'), opacity: cloneName.trim() ? 1 : 0.5 }}
      ...
      {confirmDeleteId && (
        <div style={{
          ...
          <div style={{ ...panelSx, width: 380, padding: 24, border: `1px solid ${border.medium}` }}>
