...
interface CompetitivePositionSectionProps {
  deal: Deal;
}
...
const MOCK_TRADE_AREA_COMPS: TradeAreaComp[] = [
...
const MOCK_LIKE_KIND_BENCHMARKS: LikeKindBenchmark[] = [
  { metric: 'Avg Effective Rent', subject: '$1,920', peerAvg: '$1,845', peerTop25: '$2,180', delta: '+4.1%', deltaDirection: 'up' },
  { metric: 'Occupancy', subject: '95.2%', peerAvg: '93.8%', peerTop25: '96.5%', delta: '+1.4pp', deltaDirection: 'up' },
  { metric: 'Revenue per Unit', subject: '$1,828', peerAvg: '$1,731', peerTop25: '$2,104', delta: '+5.6%', deltaDirection: 'up' },
  { metric: 'Concession Rate', subject: '2.1%', peerAvg: '3.4%', peerTop25: '1.2%', delta: '-1.3pp', deltaDirection: 'up' },
  { metric: 'Lease Renewal Rate', subject: '58%', peerAvg: '62%', peerTop25: '71%', delta: '-4pp', deltaDirection: 'down' },
  { metric: 'Traffic-to-Lease', subject: '22%', peerAvg: '19%', peerTop25: '26%', delta: '+3pp', deltaDirection: 'up' },
  { metric: 'Avg Days on Market', subject: '28', peerAvg: '34', peerTop25: '18', delta: '-6 days', deltaDirection: 'up' },
  { metric: 'Google Rating', subject: '4.2', peerAvg: '3.9', peerTop25: '4.5', delta: '+0.3', deltaDirection: 'up' },
];
...
const MOCK_AMENITY_GAPS: AmenityGap[] = [
  { amenity: 'Coworking Space', subject: false, compsPct: 43, estRentLift: 85, priority: 'high' },
  { amenity: 'EV Charging', subject: false, compsPct: 43, estRentLift: 45, priority: 'medium' },
  { amenity: 'Rooftop Deck', subject: false, compsPct: 43, estRentLift: 120, priority: 'high' },
  { amenity: 'Package Lockers', subject: true, compsPct: 71, estRentLift: 30, priority: 'low' },
  { amenity: 'Pet Spa', subject: false, compsPct: 29, estRentLift: 35, priority: 'low' },
  { amenity: 'Smart Home Tech', subject: false, compsPct: 57, estRentLift: 65, priority: 'high' },
];

const MOCK_PATTERN_ALERTS: PatternAlert[] = [
  { id: 'amenity-arms-race', pattern: 'Amenity Arms Race', description: '4 of 7 comps added coworking spaces in the last 18 months. Properties without coworking are seeing 3-5% lower traffic. Consider adding to remain competitive.', severity: 'warning', icon: '⚔️' },
  { id: 'vintage-cascade', pattern: 'Vintage Cascade', description: 'Two new Class A+ deliveries (2021-2022) are pulling top-tier renters, cascading rent pressure down to 2012-2016 vintage properties. Your 2015 vintage is in the compression zone.', severity: 'warning', icon: '🏗️' },
  { id: 'occupancy-divergence', pattern: 'Occupancy Divergence', description: 'Class B properties in this trade area maintain 97%+ occupancy vs Class A at 92-95%. Price-sensitive renter pool is strong — value-add repositioning may capture premium without vacancy risk.', severity: 'opportunity', icon: '📈' },
  { id: 'traffic-concentration', pattern: 'Traffic Concentration Risk', description: 'Colony Square Living captures 22.3% of local search traffic — dominant digital presence. Your property needs SEO/ILS investment to compete for leads.', severity: 'info', icon: '🔍' },
];
...
export const CompetitivePositionSection: React.FC<CompetitivePositionSectionProps> = ({ deal }) => {
  ...
  const sortedComps = [...MOCK_TRADE_AREA_COMPS].sort((a, b) => {
  ...
  const maxRent = Math.max(...MOCK_TRADE_AREA_COMPS.map(c => c.avgRent), SUBJECT_PROPERTY.avgRent);
  const maxTraffic = Math.max(...MOCK_TRADE_AREA_COMPS.map(c => c.trafficShare), SUBJECT_PROPERTY.trafficShare);
  const rentScale = 280 / maxRent;
  const trafficScale = 180 / maxTraffic;

  return (
    <div className="space-y-5">
      <div className="bg-stone-900 text-white rounded-xl p-4 border-l-4 border-indigo-500">
        <div className="text-[10px] font-mono text-indigo-400 tracking-widest mb-1">COMPETITIVE POSITION ANALYSIS</div>
        <div className="text-lg font-semibold">How does this property stack up against its competition?</div>
      </div>

      <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
        <div className="flex border-b border-stone-200">
          <button
            onClick={() => setActiveTab('tradeArea')}
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-stone-500 mt-0.5">{MOCK_TRADE_AREA_COMPS.length} properties within 1.5-mile radius</p>
                </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b-2 border-stone-200 text-xs text-stone-500">
                      ...
                      <th className="text-center py-2 px-3 font-medium cursor-pointer hover:text-stone-800" onClick={() => handleSort('distance')}>
                        Dist<SortIcon field="distance" />
                      </th>
                      <th className="text-center py-2 px-3 font-medium cursor-pointer hover:text-stone-800" onClick={() => handleSort('units')}>
                        Units<SortIcon field="units" />
                      </th>
                      <th className="text-center py-2 px-3 font-medium">Class</th>
                      <th className="text-center py-2 px-3 font-medium cursor-pointer hover:text-stone-800" onClick={() => handleSort('avgRent')}>
                        Avg Rent<SortIcon field="avgRent" />
                      </th>
                      <th className="text-center py-2 px-3 font-medium cursor-pointer hover:text-stone-800" onClick={() => handleSort('occupancy')}>
                        Occ%<SortIcon field="occupancy" />
                      </th>
                      <th className="text-center py-2 px-3 font-medium cursor-pointer hover:text-stone-800" onClick={() => handleSort('trafficShare')}>
                        Traffic<SortIcon field="trafficShare" />
                      </th>
                        ...
                        <tr key={idx} className="border-b border-stone-100 hover:bg-stone-50 transition-colors">
                          ...
                          <td className="text-center py-2.5 px-3">
                            <span className={`text-xs font-semibold ${
                              comp.occupancy >= 95 ? 'text-emerald-600' : comp.occupancy >= 93 ? 'text-amber-600' : 'text-red-600'
                            }`}>{comp.occupancy}%</span>
                          </td>
                          <td className="text-center py-2.5 px-3">
                            <div className="flex items-center justify-center gap-1">
                              <div className="w-16 h-1.5 bg-stone-200 rounded-full overflow-hidden">
                                <div
                                  ...
                                  style={{ width: `${(comp.trafficShare / maxTraffic) * 100}%` }}
                                />
                              </div>
                          ...
                          <td className="text-center py-2.5 px-3">
                            <div className="flex items-center justify-center gap-0.5">
                              {Object.entries(comp.amenities).map(([key, has]) => (
                                <span
                                  key={key}
                                  className={`w-2 h-2 rounded-full ${has ? 'bg-emerald-400' : 'bg-stone-200'}`}
                                  title={`${AMENITY_LABELS[key]}: ${has ? 'Yes' : 'No'}`}
                                />
                              ))}
            ...
            <div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {MOCK_LIKE_KIND_BENCHMARKS.map((bm, idx) => (
                      <tr key={idx} className="border-b border-stone-100 hover:bg-stone-50 transition-colors">
                        ...
                        <td className="text-center py-2.5 px-3">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                            bm.deltaDirection === 'up' ? 'bg-emerald-100 text-emerald-700' :
                            bm.deltaDirection === 'down' ? 'bg-red-100 text-red-700' :
            ...
            <div key={idx} className={`flex items-center justify-between p-3 rounded-lg border ${
              ...
              <div className="flex items-center gap-3 flex-1">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-stone-500">
                    {gap.compsPct}% of comps have this
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  {!gap.subject && (
                    <div className="text-[10px] text-stone-400">est. rent lift</div>
                  )}
                </div>
                {!gap.subject && (
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-6">
        ...
        <div className="relative bg-stone-50 rounded-lg border border-stone-200 p-4" style={{ height: 240 }}>
          <div className="absolute left-8 top-2 text-[9px] font-mono text-stone-400 -rotate-90 origin-left" style={{ transform: 'rotate(-90deg) translateX(-80px)' }}>
            Traffic Share %
          </div>
          <div className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[9px] font-mono text-stone-400">
            Avg Rent $/mo
          </div>
          ...
          <div className="absolute inset-0 ml-10 mb-6 mt-2 mr-4">
            ...
            {MOCK_TRADE_AREA_COMPS.map((comp, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="absolute w-3 h-3 bg-indigo-400 rounded-full border-2 border-white shadow-sm cursor-pointer hover:scale-150 transition-transform"
            ...
            {(() => {
              const sx = ((SUBJECT_PROPERTY.avgRent - 1300) / (maxRent - 1300)) * 90 + 5;
              const sy = 95 - ((SUBJECT_PROPERTY.trafficShare / maxTraffic) * 90 + 5);
              return (
                <div
                  className="absolute w-5 h-5 bg-amber-500 rounded-full border-3 border-white shadow-lg z-10"
                  style={{ left: `${sx}%`, top: `${sy}%`, transform: 'translate(-50%, -50%)', borderWidth: 3 }}
                  title={`Subject: $${SUBJECT_PROPERTY.avgRent}/mo, ${SUBJECT_PROPERTY.trafficShare}% traffic`}
                >
                  <div className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] font-bold text-amber-700 whitespace-nowrap bg-white px-1 rounded shadow-sm">
                    Subject
                  </div>
                </div>
              );
        ...
        <div className="flex items-center justify-center gap-4 mt-3 text-[10px] text-stone-500">
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 bg-amber-500 rounded-full border-2 border-white shadow-sm inline-block" /> Subject Property
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 bg-indigo-400 rounded-full border border-white shadow-sm inline-block" /> Comp Properties
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {MOCK_PATTERN_ALERTS.map((alert) => (
          <div
            key={alert.id}
            className={`rounded-xl border p-4 ${
              alert.severity === 'warning' ? 'bg-amber-50 border-amber-200' :
              alert.severity === 'opportunity' ? 'bg-emerald-50 border-emerald-200' :
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${
                    ...
                    alert.severity === 'opportunity' ? 'bg-emerald-200 text-emerald-800' :
