...
export function ZoningCapacitySection({ deal, dealId: propDealId }: ZoningCapacitySectionProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-3 text-slate-400">Loading zoning capacity...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-1">
      ...
      {saveMsg && (
        <div className={`px-4 py-2 rounded-lg text-sm ${saveMsg.includes('Failed') ? 'bg-red-500/20 text-red-300' : 'bg-green-500/20 text-green-300'}`}>
          {saveMsg}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-5">
            <h3 className="text-lg font-semibold text-white mb-4">Zoning Parameters</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Zoning Code</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Base Zoning</label>
                <select
                  value={data.base_zoning || ''}
                  onChange={(e) => updateField('base_zoning', e.target.value || null)}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Max Density (units/acre)</label>
                <input
                  type="number"
                  step="0.1"
                  value={data.max_density ?? ''}
                  onChange={(e) => updateNumField('max_density', e.target.value)}
                  placeholder="e.g. 80"
                  className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Max FAR</label>
                <input
                  type="number"
                  step="0.1"
                  value={data.max_far ?? ''}
                  onChange={(e) => updateNumField('max_far', e.target.value)}
                  placeholder="e.g. 3.2"
                  className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Max Height (ft)</label>
                <input
                  type="number"
                  value={data.max_height_feet ?? ''}
                  onChange={(e) => updateIntField('max_height_feet', e.target.value)}
                  placeholder="e.g. 65"
                  className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Max Stories</label>
                <input
                  type="number"
                  value={data.max_stories ?? ''}
                  onChange={(e) => updateIntField('max_stories', e.target.value)}
                  placeholder="e.g. 5"
                  className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div className="col-span-2">
                <label className="block text-xs font-medium text-slate-400 mb-1">Min Parking per Unit</label>
                <input
                  type="number"
                  step="0.1"
                  value={data.min_parking_per_unit ?? ''}
                  onChange={(e) => updateNumField('min_parking_per_unit', e.target.value)}
                  placeholder="e.g. 1.5"
                  className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500"
                />
              </div>
          ...
          <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-5">
            <h3 className="text-lg font-semibold text-white mb-4">Density Bonuses</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
                <div>
                  <p className="text-sm font-medium text-white">Affordable Housing Bonus</p>
                  <p className="text-xs text-slate-400">Increase density for affordable units</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-16 px-2 py-1 bg-slate-700/50 border border-slate-600 rounded text-white text-sm text-center"
                  />
                  <span className="text-xs text-slate-400">%</span>
                  <button
                    onClick={() => updateField('affordable_housing_bonus', !data.affordable_housing_bonus)}
              ...
              <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
                <div>
                  <p className="text-sm font-medium text-white">Transfer of Development Rights (TDR)</p>
                  <p className="text-xs text-slate-400">Purchase development rights from other parcels</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    type="number"
                  ...
                  <span className="text-xs text-slate-400">%</span>
                  <button
                    onClick={() => updateField('tdr_available', !data.tdr_available)}
                    className="text-slate-300 hover:text-white"
                  >
                    {data.tdr_available ? (
                      <ToggleRight size={28} className="text-green-400" />
          ...
          <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-5">
            <h3 className="text-lg font-semibold text-white mb-4">Unit Mix Distribution</h3>
            {unitMixTotal !== 100 && unitMixTotal > 0 && (
              <div className="flex items-center gap-2 mb-3 p-2 bg-amber-500/10 border border-amber-500/30 rounded-lg">
                ...
                <span className="text-xs text-amber-300">Total is {unitMixTotal}% — should equal 100%</span>
              </div>
            )}
                ...
                return (
                  <div key={key} className="flex items-center gap-4">
                    ...
                    <input
                      ...
                      className="w-20 px-2 py-1.5 bg-slate-700/50 border border-slate-600 rounded text-white text-sm text-center"
                    />
          ...
          <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-5">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Average Rent per Unit ($/month)</label>
              <input
                type="number"
                step="50"
          ...
          <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-5">
            <h3 className="text-lg font-semibold text-white mb-4">Notes & Restrictions</h3>
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500 resize-none"
            />
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-blue-900/40 to-slate-800/60 rounded-xl border border-blue-500/30 p-5">
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              <Building2 size={18} className="text-blue-400" />
              By Right
            </h3>
            <div className="text-center mb-4">
              <p className="text-4xl font-bold text-white">{formatNumber(data.max_units_by_right)}</p>
              <p className="text-xs text-slate-400 mt-1">Maximum Units</p>
            </div>
            ...
            <div className="mt-3 space-y-2 text-sm">
              {data.max_density && (
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{data.max_density} units/acre</span>
                </div>
              )}
              {data.max_far && (
                <div className="flex justify-between">
                  <span className="text-slate-400">FAR:</span>
                  <span className="text-white">{data.max_far}</span>
                </div>
              )}
              {data.max_stories && (
                <div className="flex justify-between">
                  <span className="text-slate-400">Stories:</span>
                  <span className="text-white">{data.max_stories}</span>
                </div>
              )}
            </div>
          </div>

          <div className="bg-gradient-to-br from-green-900/40 to-slate-800/60 rounded-xl border border-green-500/30 p-5">
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              <TrendingUp size={18} className="text-green-400" />
              With Incentives
            </h3>
            <div className="text-center mb-4">
              <p className="text-4xl font-bold text-green-300">{formatNumber(data.max_units_with_incentives)}</p>
              <p className="text-xs text-slate-400 mt-1">Maximum Units</p>
            </div>
            {(data.max_units_by_right && data.max_units_with_incentives) ? (
              <div className="text-center mb-3">
                <span className="inline-block px-2 py-0.5 text-xs rounded-full bg-green-500/20 text-green-300 border border-green-500/30">
            ...
            <div className="mt-3 space-y-2 text-sm">
              {data.affordable_housing_bonus && (
                <div className="flex justify-between">
                  <span className="text-slate-400">Affordable:</span>
                  <span className="text-green-300">+{data.affordable_bonus_percent || 25}%</span>
                </div>
              )}
              {data.tdr_available && (
                <div className="flex justify-between">
                  <span className="text-slate-400">TDR:</span>
                  <span className="text-green-300">+{data.tdr_bonus_percent || 15}%</span>
                </div>
              )}
            </div>
          </div>
          ...
          {(data.annual_revenue || data.avg_rent_per_unit) && (
            <div className="bg-gradient-to-br from-purple-900/40 to-slate-800/60 rounded-xl border border-purple-500/30 p-5">
              <h3 className="text-lg font-semibold text-white mb-4">Revenue Summary</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-400">Avg Rent:</span>
                  <span className="text-white">{formatCurrency(data.avg_rent_per_unit)}/mo</span>
                </div>
                {data.annual_revenue && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Annual Revenue:</span>
                      <span className="text-white">{formatCurrency(data.annual_revenue)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Pro Forma NOI:</span>
                      <span className="text-white">{formatCurrency(data.pro_forma_noi)}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t border-slate-600">
                      <span className="text-slate-300 font-medium">Est. Value (5% cap):</span>
          ...
          {data.buildable_sq_ft ? (
            <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-5">
              <h3 className="text-lg font-semibold text-white mb-3">Buildable Area</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-400">Buildable:</span>
                  <span className="text-white">{(data.buildable_sq_ft / 43560).toFixed(2)} acres</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Coverage:</span>
                  <span className="text-white">{data.coverage_ratio?.toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Gross Sq Ft:</span>
                  <span className="text-white">{formatNumber(data.buildable_sq_ft)}</span>
                </div>
              </div>
            </div>
          ) : null}
