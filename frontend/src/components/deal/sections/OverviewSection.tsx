...
export const OverviewSection: React.FC<OverviewSectionProps> = ({
  ...
  useEffect(() => {
    ...
    return () => { stopPolling?.(); };
  // Task #425: useEffect intentionally omits `loadCapitalStack`,
  // `loadEntitlementBenchmarks`, `loadEntitlements`, `loadJediScore`, and
  // `startAnalysis` — the omitted value(s) are either (a) stable references
  ...
  useEffect(() => {
    if (!deal?.id) return;
    loadMarketData();
  // Task #425: useEffect intentionally omits `loadMarketData` — the omitted
