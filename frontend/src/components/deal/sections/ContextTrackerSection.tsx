...
export const ContextTrackerSection: React.FC<ContextTrackerSectionProps> = ({ deal, dealId: propDealId }) => {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-900/50 rounded-lg p-4">
        <h3 className="text-lg font-semibold text-blue-300 mb-1">Deal Context Tracker</h3>
        <p className="text-sm text-blue-600">
          Unified view of all deal context — notes, activity, contacts, documents, dates, decisions, and risks.
        </p>
      </div>

      <div className="flex flex-wrap gap-1 bg-[#131920] rounded-lg p-1">
        {TABS.map(tab => {
          const Icon = tab.icon;
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium transition-all ${
                activeTab === tab.id
                  ...
                  : 'text-[#6B7585] hover:text-[#9EA8B4] hover:bg-[#0F1319]'
              }`}
            >
              <Icon size={14} />
              {tab.name}
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h4 className="font-semibold text-[#E8E6E1]">Notes ({notes.length})</h4>
      ...
      {showForm && (
        <div className="bg-[#0F1319] border border-[#1e2a3d] rounded-lg p-4 space-y-3">
          <input type="text" placeholder="Note title (optional)" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
            className="w-full px-3 py-2 border border-[#253347] rounded-lg text-sm" />
          <textarea placeholder="Write your note..." value={form.content} onChange={e => setForm({ ...form, content: e.target.value })}
            className="w-full px-3 py-2 border border-[#253347] rounded-lg text-sm h-24 resize-none" />
          <div className="flex gap-3 items-center">
            <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}
              className="px-3 py-1.5 border border-[#253347] rounded-lg text-sm">
              {categories.map(c => <option key={c} value={c}>{c.replace('-', ' ')}</option>)}
            </select>
            <label className="flex items-center gap-1.5 text-sm text-[#9EA8B4]">
              <input type="checkbox" checked={form.pinned} onChange={e => setForm({ ...form, pinned: e.target.checked })} /> Pin
            </label>
            <div className="flex-1" />
            <button onClick={() => setShowForm(false)} className="px-3 py-1.5 text-[#6B7585] text-sm">Cancel</button>
      ...
      {notes.length === 0 ? (
        ...
        <div className="space-y-3">
          {notes.map((note: any) => (
            <div key={note.id} className={`bg-[#0F1319] border rounded-lg p-4 ${note.pinned ? 'border-yellow-300 bg-[#1a1200]/30' : 'border-[#1e2a3d]'}`}>
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  {note.pinned && <span className="inline-flex items-center gap-1 text-xs text-yellow-600 mb-1"><Pin size={12} /> Pinned</span>}
                  {note.title && <h5 className="font-medium text-[#E8E6E1]">{note.title}</h5>}
                  <p className="text-sm text-[#9EA8B4] mt-1 whitespace-pre-wrap">{note.content}</p>
                  <div className="flex items-center gap-3 mt-2">
                    {note.category && <span className="text-xs px-2 py-0.5 bg-[#131920] text-[#9EA8B4] rounded">{note.category}</span>}
...
function ActivityTab({ dealId }: { dealId: string }) {
  ...
  return (
    <div className="space-y-4">
      <h4 className="font-semibold text-[#E8E6E1]">Activity Timeline ({activities.length})</h4>
      {activities.length === 0 ? (
          ...
          {activities.map((a: any, i: number) => (
            <div key={a.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className="w-8 h-8 rounded-full bg-[#022c22] text-green-600 flex items-center justify-center"><Clock size={14} /></div>
                {i < activities.length - 1 && <div className="w-px h-full bg-[#1e2a3d] my-1" />}
              </div>
              <div className="pb-4 flex-1">
                <p className="text-sm font-medium text-[#E8E6E1]">{a.title}</p>
                {a.description && <p className="text-xs text-[#6B7585] mt-0.5">{a.description}</p>}
                <div className="flex items-center gap-2 mt-1">
                  {a.module_name && <span className="text-xs px-2 py-0.5 bg-[#0d1e3d] text-blue-600 rounded">{a.module_name}</span>}
...
function ContactsTab({ dealId }: { dealId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h4 className="font-semibold text-[#E8E6E1]">Contacts ({contacts.length})</h4>
        ...
        <div className="bg-[#0F1319] border border-[#1e2a3d] rounded-lg p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <input type="text" placeholder="Name *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
              className="px-3 py-2 border border-[#253347] rounded-lg text-sm" />
          ...
          <textarea placeholder="Notes about this contact..." value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
            className="w-full px-3 py-2 border border-[#253347] rounded-lg text-sm h-16 resize-none" />
      ...
      {contacts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {contacts.map((c: any) => (
            <div key={c.id} className={`bg-[#0F1319] border rounded-lg p-4 ${c.is_primary ? 'border-purple-300 ring-1 ring-purple-100' : 'border-[#1e2a3d]'}`}>
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    <h5 className="font-medium text-[#E8E6E1]">{c.name}</h5>
                    {c.is_primary && <span className="text-xs px-1.5 py-0.5 bg-[#1a0d3d] text-purple-400 rounded">Primary</span>}
                  </div>
                  <p className="text-xs text-[#6B7585] mt-0.5">{c.role}{c.company ? ` at ${c.company}` : ''}</p>
                </div>
                <button onClick={() => deleteContact(c.id)} className="text-gray-400 hover:text-red-500 p-1"><Trash2 size={14} /></button>
              </div>
              <div className="mt-2 space-y-1">
                {c.email && <p className="text-xs text-[#9EA8B4]">✉ {c.email}</p>}
                {c.phone && <p className="text-xs text-[#9EA8B4]">☎ {c.phone}</p>}
                {c.notes && <p className="text-xs text-[#6B7585] mt-1 italic">{c.notes}</p>}
              </div>
            </div>
          ))}
        </div>
...
function DocumentsTab({ dealId }: { dealId: string }) {
  ...
  const categoryColors: Record<string, string> = {
    'due-diligence': 'bg-[#0d1e3d] text-blue-400', financial: 'bg-[#022c22] text-green-400',
    legal: 'bg-[#1a0d3d] text-purple-400', plans: 'bg-[#1a1200] text-amber-400',
    photos: 'bg-pink-100 text-pink-700', environmental: 'bg-emerald-100 text-emerald-700',
    appraisal: 'bg-indigo-100 text-indigo-400', other: 'bg-[#131920] text-[#9EA8B4]',
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h4 className="font-semibold text-[#E8E6E1]">Documents ({documents.length})</h4>
      ...
      {documents.length === 0 ? (
        ...
        <div className="space-y-2">
          {documents.map((d: any) => (
            <div key={d.id} className="bg-[#0F1319] border border-[#1e2a3d] rounded-lg p-3 flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#131920] flex items-center justify-center"><FolderOpen size={18} className="text-gray-400" /></div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#E8E6E1] truncate">{d.original_filename}</p>
                <div className="flex items-center gap-2 mt-0.5">
                  {d.category && <span className={`text-xs px-1.5 py-0.5 rounded ${categoryColors[d.category] || 'bg-[#131920] text-[#9EA8B4]'}`}>{d.category}</span>}
...
function FinancialTab({ dealId }: { dealId: string }) {
  return (
    <div className="space-y-4">
      <h4 className="font-semibold text-[#E8E6E1]">Financial Snapshot</h4>
      <div className="bg-[#0F1319] border border-[#1e2a3d] rounded-lg p-6">
        <p className="text-sm text-[#6B7585] mb-4">Quick financial overview pulled from deal data and modules.</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={m.label} className="text-center p-3 bg-[#0F1319] rounded-lg">
              <p className="text-xs text-[#6B7585]">{m.label}</p>
              <p className="text-lg font-bold text-[#E8E6E1] mt-1">{m.value}</p>
...
function DatesTab({ dealId }: { dealId: string }) {
  ...
  const statusColors: Record<string, string> = {
    upcoming: 'text-blue-600 bg-[#0d1e3d]', completed: 'text-green-600 bg-[#022c22]',
    missed: 'text-red-400 bg-[#1c0a0a]', cancelled: 'text-gray-400 bg-[#0F1319]',
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h4 className="font-semibold text-[#E8E6E1]">Key Dates ({dates.length})</h4>
      ...
      {dates.length === 0 ? (
        ...
        <div className="space-y-2">
          {dates.map((d: any) => {
            ...
            return (
              <div key={d.id} className={`bg-[#0F1319] border rounded-lg p-3 flex items-center gap-3 ${isPast ? 'border-red-800/50' : 'border-[#1e2a3d]'}`}>
                <button onClick={() => toggleStatus(d)} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                  d.status === 'completed' ? 'bg-green-500 border-green-500 text-white' : 'border-[#253347]'
                }`}>
                  {d.status === 'completed' && <CheckCircle size={14} />}
                </button>
                <div className="flex-1">
                  <p className={`text-sm font-medium ${d.status === 'completed' ? 'text-gray-400 line-through' : 'text-[#E8E6E1]'}`}>{d.title}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`text-xs px-1.5 py-0.5 rounded ${statusColors[d.status] || 'bg-[#131920] text-[#9EA8B4]'}`}>{d.date_type}</span>
...
function DecisionsTab({ dealId }: { dealId: string }) {
  ...
  const statusColors: Record<string, string> = {
    approved: 'bg-[#022c22] text-green-400', rejected: 'bg-[#1c0a0a] text-red-400',
    pending: 'bg-yellow-100 text-yellow-700', tabled: 'bg-[#131920] text-[#9EA8B4]',
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h4 className="font-semibold text-[#E8E6E1]">Decision Log ({decisions.length})</h4>
        ...
        <div className="bg-[#0F1319] border border-[#1e2a3d] rounded-lg p-4 space-y-3">
          <div className="grid grid-cols-3 gap-3">
            <input type="text" placeholder="Decision title *" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
              className="col-span-1 px-3 py-2 border border-[#253347] rounded-lg text-sm" />
      ...
      {decisions.length === 0 ? (
        ...
        <div className="space-y-3">
          {decisions.map((d: any) => (
            <div key={d.id} className="bg-[#0F1319] border border-[#1e2a3d] rounded-lg p-4">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h5 className="font-medium text-[#E8E6E1]">{d.title}</h5>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${statusColors[d.status] || 'bg-[#131920] text-[#9EA8B4]'}`}>{d.status}</span>
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-[#6B7585]">{d.decision_type}</span>
                    {d.decision_date && <span className="text-xs text-gray-400">{new Date(d.decision_date).toLocaleDateString()}</span>}
                  </div>
                  {d.rationale && <p className="text-sm text-[#9EA8B4] mt-2">{d.rationale}</p>}
                  {d.impact_description && <p className="text-sm text-[#6B7585] mt-1 italic">{d.impact_description}</p>}
                </div>
...
function RisksTab({ dealId }: { dealId: string }) {
  ...
  const severityColors: Record<string, string> = {
    high: 'bg-[#1c0a0a] text-red-400 border-red-800/50', medium: 'bg-yellow-100 text-yellow-700 border-yellow-200',
    low: 'bg-[#022c22] text-green-400 border-green-800/50',
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h4 className="font-semibold text-[#E8E6E1]">Risk Register ({risks.length})</h4>
      ...
      {risks.length === 0 ? (
        ...
        <div className="space-y-3">
          {risks.map((r: any) => {
            ...
            return (
            <div key={r.id} className={`bg-[#0F1319] border rounded-lg p-4 ${severityColors[severity] || 'border-[#1e2a3d]'}`}>
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <AlertTriangle size={16} className={severity === 'high' ? 'text-red-500' : severity === 'medium' ? 'text-yellow-500' : 'text-green-500'} />
                    <h5 className="font-medium text-[#E8E6E1]">{r.title}</h5>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${severityColors[severity] || 'bg-[#131920] text-[#9EA8B4]'}`}>
                      {severity} severity
                    </span>
                  </div>
                  <div className="flex items-center gap-3 mt-1 text-xs text-[#6B7585]">
                    <span>{r.category}</span>
                    <span>Impact: {r.impact}</span>
                    <span>Likelihood: {r.likelihood}</span>
                    <span className={`px-1.5 py-0.5 rounded ${r.status === 'active' ? 'bg-[#1c0a0a] text-red-400' : 'bg-[#022c22] text-green-600'}`}>{r.status}</span>
                  </div>
                  {r.description && <p className="text-sm text-[#9EA8B4] mt-2">{r.description}</p>}
                  {r.mitigation_strategy && (
                    <div className="mt-2 p-2 bg-[#0d1e3d] rounded text-sm text-blue-400">
...
function EmptyState({ message, icon: Icon }: { message: string; icon: React.FC<any> }) {
  return (
    <div className="text-center py-12 bg-[#0F1319] border border-[#1e2a3d] rounded-lg">
      <Icon size={40} className="mx-auto text-gray-300 mb-3" />
      <p className="text-sm text-[#6B7585] max-w-md mx-auto">{message}</p>
    </div>
  );
}
