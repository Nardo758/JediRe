...
import { BubbleLayerRenderer } from './BubbleLayerRenderer';
import { OverlayLayerRenderer } from './OverlayLayerRenderer';
import * as turf from '@turf/turf';

interface LayerRendererFullProps {
  layers: LayerConfig[];
  mapId: string;
  onMarkerClick?: (dataPoint: LayerDataPoint) => void;
}

export const LayerRendererFull: React.FC<LayerRendererFullProps> = ({
  layers,
  mapId,
  onMarkerClick
}) => {
  const [layerData, setLayerData] = useState<Record<string, LayerDataPoint[]>>({});
  const [overlayGeojson, setOverlayGeojson] = useState<Record<string, any>>({});
  ...
  useEffect(() => {
    const fetchLayerData = async () => {
      ...
      for (const layer of visibleLayers) {
        ...
        try {
          if (layer.layer_type === 'overlay') {
            // Fetch overlay geojson (different endpoint/format)
            // TODO: Implement overlay data fetching
            console.log(`[LayerRenderer] Overlay layers not yet implemented for source: ${layer.source_type}`);
          } else {
    ...
    fetchLayerData();
  // Task #425: useEffect intentionally omits `layerData` — the omitted
    ...
    <>
      {sortedLayers.map(layer => {
        ...
        switch (layer.layer_type) {
          ...
          case 'bubble':
            return data ? (
              <BubbleLayerRenderer key={layer.id} layer={layer} data={data} />
            ) : null;
          ...
          case 'overlay':
            return overlayGeojson[layer.id] ? (
              <OverlayLayerRenderer
                key={layer.id}
                layer={layer}
                geojson={overlayGeojson[layer.id]}
              />
            ) : null;
...
export default LayerRendererFull;
