...
| 2026-05-29 | **Phase 5 shadow path fully operational — production data ops + bug fixes (this session).** (1) `USE_NEW_PROPERTY_SCHEMA_VALUATION_COMPS=shadow` set in shared env. (2) `synthesize-implied-cap-rates.ts` run live: Pass 2 wrote 6,026 implied_cap_rate rows (avg 5.76%, 32 out-of-range skipped), Pass 3 backfilled 148,641 `properties` rows with lat/lng from comps. (3) Coordinate sync: `latitude=lat, longitude=lng` applied to 148,675 rows so valuation-grid's `subject.latitude` guard fires correctly. (4) Ghost comp-set migration applied (DELETE 0 — no true ghosts). (5) Shadow path fixes: (a) radius floor raised 10→25 miles (Cobb comps ~18 miles from south-Atlanta deals); (b) unit-count filters removed from shadow call — broker_comp properties lack `units` data, causing all 544 nearby comps to fail `p.units >= 50`; (c) backtest 3-second wait before shadow count query to handle fire-and-forget timing. Final backtest: S1 gold set 3/3 PASS (4/5 active each). Shadow log: Atlanta MF #1 new_path_n=500 p50=6.20%; Atlanta MF #2 new_path_n=447 p50=6.23%; Jacksonville insufficient (no FL comps — structural gap). Next: monitor shadow log for 7 days then promote flag to `canary`. |
