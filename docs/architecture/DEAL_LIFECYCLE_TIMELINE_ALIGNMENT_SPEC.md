...
- **Data inflow:** monthly operating statements, rent roll, occupancy. **This is where lease-up trajectory data is generated** — the data the owned portfolio currently lacks.
...
- **Correlation Engine Phase 1B** (blocked on zero stabilization rows + no outcome schema) is *fed* by Phases E→H. The lifecycle overlay is the data source that unblocks it — but only for deals that run the full lifecycle in-platform. Existing owned portfolio predates this and lacks the trajectory; new in-platform deals generate it.
