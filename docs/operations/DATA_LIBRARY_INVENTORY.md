...
| 10 | **Line-item benchmarks / archive distributions** | `seed-opex-benchmarks.ts` (seeded 2025-12-31) | `line_item_benchmarks` (263 rows — national + vintage-band-stratified + top-10 MSAs, Class A/B/C, 14 line items); `archive_assumption_benchmarks` (209 rows — Class A/B/C, canonical assumption names + closed-deal derivation path) | `fetch_line_item_benchmarks`, `fetch_archive_assumption_distribution`, `fetch_archive_achievement_vs_assumption` — all three tools exist and call these tables | Per-unit OpEx P10/P50/P90 ranges; assumption vs. achievement gap for closed deals | **Seeded — G7 closed. National Class B benchmarks match at `national_class` bucket. Top-10 MSA Class B benchmarks match at `msa_class_vintage` bucket. Vintage-band rows (pre-1990/1990-2005/2006-2015/2016+) at national Class B level.** |
...
|-------|--------|-----------|-----|
| `line_item_benchmarks` | Correct (30+ columns, P10–P90 distributions, per_unit/pct_egi/yoy) | **263 rows** (seeded 2025-12-31 via `seed-opex-benchmarks.ts`) | **G7 CLOSED.** Covers 14 line items × 3 classes (A/B/C) at national level (null deal_type for bucket matching) + null-class catch-all + Class B at top-10 MSAs (deal_type='existing') + Class B vintage-band rows (pre-1990, 1990-2005, 2006-2015, 2016+) at national level. `fetch_line_item_benchmarks` returns non-empty for Class A/B/C garden deals. NOTE: No state-level rows yet (#1113 tracks this). |
| `archive_assumption_benchmarks` | Correct (assumed_median, achieved_median, gap_bps) | **209 rows** (190 pre-existing + 19 seeded) | **G7 CLOSED (partial).** Canonical `vacancy_pct`, `concessions_pct`, `bad_debt_pct`, `management_fee_pct`, `exit_cap_rate_pct`, `expense_ratio_pct`, `rent_growth_pct` added at broadest null-vintage/null-submarket bucket for Class A/B/C with n_samples ≥ 300 (synthetic industry benchmarks). Derivation from closed deals (`data_library_assets WHERE sale_date IS NOT NULL`) wired into seed script — currently yields 0 qualifying rows because platform has only 1 closed deal (n_samples < 5 guard). Derivation path runs automatically on re-execution as more deals close. `fetch_archive_assumption_distribution` now returns results at broadest bucket. |
...
| **`line_item_benchmarks` seeded (G7)** | **PASSING** — 263 rows (national Class A/B/C + vintage-band Class B + top-10 MSAs Class B). For FL deals: national_class bucket resolves (no FL-specific rows; tracked #1113). | Pass if `SELECT COUNT(*) FROM line_item_benchmarks WHERE asset_class = 'B' AND state IS NULL` > 0 |
