...
- **Georgia sale comp enrichment** (Task #1476): `cd backend && npm run script:enrich-georgia-comps`. Re-runnable 6-step pipeline: (1) ArcGIS county ingestion → `property_info_cache` (units, sqft, year_built, stories); (2) backfill `market_sale_comps` attributes via `georgia_property_sales` join; (3) re-run `promoteGeorgiaSales` ETL; (4) `enrichCapitalMarkets` (back-solves units/PPU/cap-rate for $5M+ transactions); (5) mark comps with units < min-units as `qualified=false`; (6) validation report with Bishop comp pool count. Key flags: `--county=Fulton,Gwinnett` (default: all 12 target counties), `--dry-run`, `--min-units=4`, `--skip-ingest`, `--skip-promote`, `--skip-enrich`. The promote step (STEP 3) is the slowest (~5–15 min per county on 300K+ rows) — use `--skip-promote` when only running the enrich/backfill phases. Script: `backend/scripts/enrich-georgia-comps.ts`. **County status:** Core 5 (Cobb, Gwinnett, DeKalb, Fulton, Clayton) have ArcGIS ingestion services. Inner ring (Cherokee, Forsyth, Henry, Douglas, Fayette, Paulding, Rockdale) are in VALID_COUNTIES and wired into the promote/enrich pipeline — their ingest step is skipped until endpoints are confirmed. Henry County confirmed unreachable from cloud IPs.
- **Georgia sale comp time-series check**: `cd backend && npx ts-node --transpile-only scripts/synthesize-implied-cap-rates.ts --time-series`. Runs Passes 1–3 then prints quarterly P50 cap rates for two Atlanta reference points (25-mile radius, 20-quarter lookback) to validate date preservation and trend visibility.
- **Properties stories backfill**: `cd backend && npx ts-node --transpile-only scripts/backfill-properties-stories.ts`. One-time script that fills `properties.stories = NULL` (or 0) rows from already-cached county assessor data in `property_info_cache`, matched on `parcel_id`. Safe to re-run (skips rows where stories > 0). Supports `--dry-run`. Logs count of updated rows and each affected property.
- **Vault enrichment backfill** (populates Property Profile card): `cd backend && npx ts-node --transpile-only scripts/vault-enrich-deals.ts`. Runs the full intake pipeline (municipal ArcGIS lookup → regulatory → web search → Google Places) for all deal-linked properties that have no `property_descriptions` vault row. Resolves real county parcel IDs and writes them back to `properties.parcel_id`. Flags: `--dry-run` (preview only), `--deal-id=<uuid>` (single deal). After running, the Property Profile card and `fetch_property_vault_intel` agent tool will show municipal data, amenity flags, and narrative for enriched deals. Properties in unsupported counties/states will land in `blocked_needs_user` state (no vault row) — this is expected.
...
-   **P11 — Grep-before-claim:** Before stating that a code path, service reference, or data flow exists in current implementation, verify by direct search in the codebase. Names, section headings, design specs, and prior conversation are not evidence of implementation. (Rationale: T-CONF-2 found a described dual-surface that neither component had implemented — zero references to `deal_market_intelligence` in either file. Architectural-intent conflation is indistinguishable from real conflation without a grep.)
-   **HMR false-positive errors:** Vite HMR collisions during active editing can produce runtime errors (including import/module errors) that disappear on hard refresh. Before treating any error as production-relevant, let edits settle, hard-refresh the preview, and confirm the error persists. This is especially common when editing a file that another file is simultaneously importing.
...
-   **Owned portfolio identification — canonical flag:** `deal_monthly_actuals.is_portfolio_asset = TRUE` is the single source of truth for owned-portfolio rows. The prior implicit convention (`deal_id IS NULL`) and `property_operating_data.is_owned` are both superseded and deprecated. To add a new owned portfolio property, insert `deal_monthly_actuals` rows with `is_portfolio_asset = TRUE` and `deal_id = NULL`; agent tooling (`fetch_owned_asset_actuals`, `fetch_owned_asset_opex_ratios`) will pick it up automatically. Do not write to `property_operating_data.is_owned`.
-   **Owned portfolio composition — verified (2026-05-31):** The actual owned portfolio is exactly **one property**: **Highlands at Sweetwater Creek (p2122)** — 2789 Satellite Blvd, Duluth GA 30096, property_id `7ea31caf-f070-43eb-9fd1-fe08f7123701`, deal_id `eaabeb9f-830e-44f9-a923-56679ad0329d`. 65 monthly rows, Dec 2021–Apr 2026 (BPI Accrual+GAAP). "Parkway 290 Apartments" (Frisco TX) and "Cedar Hills at McKinney" (McKinney TX) were seeded placeholder properties — confirmed NOT owned, deleted 2026-05-31. There is no Jacksonville property, no Atlanta Midtown property, and no data from 2018 or 2020.
