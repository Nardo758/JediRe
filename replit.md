...
-   **Municipal Zoning Database:** 32 ArcGIS REST API connectors (26 verified, 6 unverified) across 13 SE + TX states. FL coverage: 10 county-level + 8 city-level connectors (Miami, Tampa, Hollywood, St. Petersburg, Hialeah, West Palm Beach, Coral Gables, Cape Coral). Miami Beach and Boca Raton use county fallback (no city zoning endpoints). 789 FL zoning districts ingested (1,191 total across all states). 63 municipalities seeded (28 FL).
...
-   **Florida Benchmark Ingestion Pipeline:** County-configurable ingestion from FL public ArcGIS APIs. Supports Miami-Dade (via `MD_ZoningRecords/MapServer/0`, per-year queries due to server limitations) and Hillsborough (via `DSD_Zoning_Hearings/FeatureServer/0` + `PermitsPlus` permits). Filters case types to relevant entitlements (rezonings, variances, CUPs, site plans), maps to `benchmark_projects` with estimated phase durations by entitlement type. Current data: ~1,168 MDC records (485 rezone, 465 variance, 218 site plan) + ~3,200 Hillsborough records (1,148 rezone, 1,122 variance, 503 site plan, 318 CUP, 109 permits). Endpoints: `POST /api/v1/benchmark-timeline/ingest/florida` (single county), `POST /api/v1/benchmark-timeline/ingest/florida/all` (all counties). CLI: `npx tsx backend/src/scripts/fetch-fl-benchmarks.ts --county=miami-dade|--all|--list`. Service: `backend/src/services/florida-benchmark-ingestion.service.ts`.
...
-   **Time-to-Shovel Real Data Pipeline:** Monte Carlo timeline service (`monte-carlo-timeline.service.ts`) `loadBenchmarks()` now queries real `benchmark_projects` data (falls back to synthetic when <3 records). New endpoints: `GET /benchmarks` (real percentile stats by entitlement type), `GET /detailed-steps` (per-phase P25/P50/P75/P90 from real data), `GET /jurisdiction-comparison` (municipality rankings with carry cost deltas). Frontend `TimeToShovelTab.tsx` fetches all three endpoints and shows "Real Data (N projects)" / "Estimated" badges per section.
