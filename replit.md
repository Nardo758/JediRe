...
-   **Split FAR & Zoning Nuances (Implemented):** Migration 052 adds `residential_far`, `nonresidential_far`, `density_method` (units_per_acre|far_derived), `height_buffer_ft`, `height_beyond_buffer_ft` columns to `zoning_districts`. MRC-2 seeded: residential FAR 1.49, nonresidential 2.50, combined 3.20, FAR-derived density, 150 ft height buffer / 225 ft beyond. Base-district fallback inherits all new columns (MRC-2-C → MRC-2). Building envelope service selects applied FAR based on deal type (residential → 1.49, commercial → 2.50, mixed-use → 3.20 combined). FAR-derived density: units = GBA × 0.85 efficiency ÷ avg unit size (no density cap). Dev Capacity tab shows split FAR in GBA Breakdown ("Residential FAR applied" badge), all three FARs in Zoning Standards grid, and conditional variant (-C) warning callout. Property Boundary sidebar shows "1.49 res / 2.50 nonres (3.20 combined)". **Remaining research:** FAR denominator uses net lot area (after dedications/easements/ROW), residential uses get open space bonus, density bonuses (+0.5× FAR for affordable housing ≥20%, +2 SF per 1 SF ground-floor retail, +2 SF per 1 SF open space above minimum), open space 20% of net lot area for sites > 0.5 acre, parking specifics (eating/drinking 1/300 SF, office no minimum).
