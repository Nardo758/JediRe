...
-   **Parcel-Level Zoning Lookup:** Automatic zoning code detection via ArcGIS spatial query when a property boundary is saved. Endpoint `GET /api/v1/zoning/parcel-lookup?lat=&lng=` queries the city's ArcGIS REST API (e.g., Atlanta GIS) to get the exact zoning classification for the parcel's coordinates. Falls back to generic district list when no API is available. Frontend (`ZoningConfirmTab`) tries parcel-lookup first, pre-selects the matching district, and shows the data source (e.g., "Atlanta GIS"). Configured for 10 verified cities via `CITY_APIS` in `municipal-api-connectors.ts`.
