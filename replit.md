...
-   **Dev Capacity Redesign (2026-02-28):** Consolidated Conditional Variant, Constraint Set, Controlling Factor, and Calculation Breakdowns into a single "Zoning Constraint Matrix" table. Removed redundant Path Comparison Table and Entitlement Strategy 3-column cards. Added zoning code input to scenarios (auto-fills from district lookup, all fields editable). Scenarios now show top 3 "Reference Projects" from real benchmark data sorted by highest density. Market Reality Check expanded with FAR and Lot Coverage comparison bars alongside density, plus enriched project cards showing all metrics.
...
-   **Dev Capacity Market Reality Check:** Grouped-by-zoning-code cards on Development Capacity tab. Projects grouped by `zoningTo` code, each code gets its own card with: header (code + project count pill, "YOUR CODE" badge on deal's own code), density/FAR/lot coverage comparison bars ("Code allows" bar only on deal's code), utilization badges, summary stats (avg building SF, avg lot), and compact project rows. Cards sorted: deal's code first, then by avg density descending. Only top 3 codes visible by default; "Show N more" toggle reveals the rest. Bogus zoning codes (SITE-PLAN, Drive-Thru-Allowed, etc.) filtered via regex and grouped under "Other". Project rows show docket number (mono font) and clickable "PDF" link to ordinance URL where available. Backend `cleanProjectMapper` exposes `docketNumber` and `ordinanceUrl` from benchmark data. Positioned after the Zoning Constraint Matrix and before Development Scenarios. Scope label shows data source breakdown. Sparse/nearby-only data availability banners preserved. "Projects That Left [code]" section at bottom.
-   **Dev Capacity Rezone as Scenario:** Removed standalone "Rezone Opportunity Detected" violet banner and "Entitlement Strategy Insight" block. Rezone opportunity auto-creates a development scenario named "Rezone to [targetCode]" when `rezoneAnalysis.bestTarget` is detected (loadData, lines ~281-303). Scenario cards with rezone names show a violet "REZONE OPPORTUNITY" badge (line ~1340). Removed PathSelection component import and rendering — users pick scenarios directly from the Development Scenarios section.
