...
-   **Building Envelope Service:** Universal calculation engine for 6 property types, determining buildable area, GFA, capacity by constraint, and performing Highest & Best Use analysis. Includes Claude-powered optimization recommendations. Wired to the Dev Capacity tab (tab 4) via `GET /api/v1/deals/:dealId/development-capacity`, which pulls boundary area from `property_boundaries`, confirmed zoning from `deal_zoning_confirmations`, looks up district standards from `zoning_districts` (with per-field base-district fallback for conditional variants like MRC-2-C → MRC-2), and runs `BuildingEnvelopeService.calculateEnvelope()` to produce max units, GFA, floors, parking, and capacity-by-constraint breakdown showing the binding constraint. Frontend auto-loads on tab open — no manual address input.
