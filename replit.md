...
- **Georgia sale comp enrichment** (Task #1476): `cd backend && npm run script:enrich-georgia-comps`. Re-runnable 6-step pipeline: (1) ArcGIS county ingestion → `property_info_cache` (units, sqft, year_built, stories); (2) backfill `market_sale_comps` attributes via `georgia_property_sales` join; (3) re-run `promoteGeorgiaSales` ETL; (4) `enrichCapitalMarkets` (back-solves units/PPU/cap-rate for $5M+ transactions); (5) mark comps with units < min-units as `qualified=false`; (6) validation report with Bishop comp pool count. Key flags: `--county=Fulton,Gwinnett` (default all 4), `--dry-run`, `--min-units=4`, `--skip-ingest`, `--skip-promote`, `--skip-enrich`. The promote step (STEP 3) is the slowest (~5–15 min per county on 300K+ rows) — use `--skip-promote` when only running the enrich/backfill phases. Script: `backend/scripts/enrich-georgia-comps.ts`.
