...
-   **Dev Capacity Market Reality Check:** UI card on Development Capacity tab showing "What the Code Allows vs What Developers Actually Built" — comparison bars, utilization badges, per-project cards with address/lot/units/SF/assessed value, and "Projects that left [code]" section. Now shows both code-matched projects (labeled with zoning code) and nearby development activity (geography-based projects with their zoning codes displayed prominently). Scope label shows e.g. "1 in MRC-2-C + 16 nearby". Graceful empty states at all data levels.
