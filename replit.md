...
-   **Property & Zoning Module:** Embedded within the Deal Detail page, offering a 5-tab progressive workflow for zoning analysis, including Boundary & Zoning, Dev Capacity (with 4-path selector), Regulatory Risk, Zoning Comparator, and Time-to-Shovel.
-   **Zoning Verification Pipeline:** A verification-first architecture with dedicated tables and frontend components for source citation, verification, and user trust gating.
...
-   **Highest & Best Use (HBU) Analysis:** An endpoint calculates and ranks 6 property types by estimated value based on zoning constraints.
-   **Entitlement Strategy (By-Right/Variance/Rezone):** An endpoint calculates three entitlement paths with associated risk, success rate, timeline, and cost.
