...
## FAILURE-MODE TRACE — the unhappy path

The hop table above traces the happy path. This section traces what happens when a hop fails.
Failure modes are only concrete inside a flow, which is why they live here and not in the
plumbing audit. For each, find the handling code (`file:line`) and answer: does it degrade
visibly, or fail silent / corrupt / crash? "No handler found" is itself a finding.

1. **Vendor API failure.** A Research tool (`fetch_rentcast_comps`, `fetch_parcel`, etc.) gets a
   500 or times out mid-run. Does the run abort cleanly, skip-and-flag the missing input, or write
   a partial DealContext that downstream agents read as complete? A partial context consumed as
   whole is the no-silent-stale violation, surfaced as a real failure rather than a missing dep.
2. **Agent run error.** Research throws partway through assembly. Is the partial DealContext rolled
   back / marked incomplete, or left in the DB looking finished? Check the `agent_runs.status`
   write path — does a crash leave status `running` forever, or transition to `failed`?
3. **Inngest retry double-charge.** Spec says `retries: 3`. Confirm a retried run does NOT bill the
   user's credits twice (metering must be idempotent per `agent_run_id`, not per attempt). This is
   a money bug, not just an ops bug.
4. **LLM/Tavily failure.** Claude API rate-limit or Tavily outage mid-run. Confirm graceful failure
   with a coherent user-facing message, not a silent hang or a raw stack trace returned to the chat
   channel.
5. **Timeout.** Research Agent's 5-minute target is exceeded. What does the user see — a timeout
   message, or silence? Is the run aborted and credits refunded, or left to run and bill?
6. **Concurrency.** Two chat users trigger Research on the same submarket simultaneously. Any shared
   mutable state, cache race, or write collision? (Recall the Ticket A write-race class.)

Report each as HANDLED (cite the handler) / UNHANDLED (cite the gap) / UNVERIFIED.
...
7. **Failure-mode findings:** the six failure paths, each HANDLED / UNHANDLED / UNVERIFIED. Flag the
   retry double-charge (#3) and partial-DealContext (#1/#2) results explicitly — those are the two
   that cost money or corrupt state.
8. **Doc-vs-code gaps:** every place a spec claimed something the code doesn't have (start with the
