/**
 * JEDI RE: Data Pipeline Schema (Drizzle ORM)
 * =============================================
 * Drizzle schema definitions for the 5 core data pipeline tables.
 * Place in: backend/src/db/schema/dataPipeline.ts
 * 
 * Tables: msas, submarkets, dealMonthlyActuals, dataUploads, 
 *         uploadTemplates, proformaTemplates, proformaSnapshots
 * 
 * Note: The `properties` table likely already exists in your schema.
 * If so, skip that section and reference your existing properties table.
 */
...
export const msas = pgTable('msas', {
  ...
  cbsaCode: varchar('cbsa_code', { length: 10 }).unique().notNull(),
  name: varchar('name', { length: 200 }).notNull(),
  state: varchar('state', { length: 2 }).notNull(),
  population: integer('population'),
  medianHhi: numeric('median_hhi', { precision: 12, scale: 2 }),
...
export const submarkets = pgTable('submarkets', {
  id: uuid('id').primaryKey().defaultRandom(),
  msaId: uuid('msa_id').notNull().references(() => msas.id),
  name: varchar('name', { length: 200 }).notNull(),
  boundary: jsonb('boundary'), // GeoJSON polygon
  // Submarket vitals — feeds M05 Market Vitals, F26 submarket rank
  avgRent: numeric('avg_rent', { precision: 10, scale: 2 }),
  vacancyRate: numeric('vacancy_rate', { precision: 5, scale: 3 }),
  absorptionRate: numeric('absorption_rate', { precision: 10, scale: 2 }),
  rentGrowthYoy: numeric('rent_growth_yoy', { precision: 5, scale: 3 }),
  popGrowthYoy: numeric('pop_growth_yoy', { precision: 5, scale: 3 }),
  submarketRank: numeric('submarket_rank', { precision: 5, scale: 2 }), // F26 percentile
  vitalsUpdated: timestamp('vitals_updated', { withTimezone: true }),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow(),
}, (table) => ({
  msaIdx: index('idx_submarkets_msa').on(table.msaId),
  rankIdx: index('idx_submarkets_rank').on(table.submarketRank),
}));
...
// =============================================================================
// 4. PROFORMA TEMPLATES — Reusable Assumption Sets (3-Layer)
// =============================================================================
...
export const proformaSnapshots = pgTable('proforma_snapshots', {
  ...
  layer1Baseline: jsonb('layer1_baseline').default({}),
  layer2Adjusted: jsonb('layer2_adjusted').default({}),
  layer3User: jsonb('layer3_user').default({}),
...
// =============================================================================
// 6. TYPE EXPORTS (for use in services)
// =============================================================================
