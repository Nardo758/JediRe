...
function MiniChart({ data, color = T.green, h = 80 }) {
  const mx = Math.max(...data), mn = Math.min(...data), r = mx - mn || 1;
...
function ThresholdVal({ value, thresholds, invert }) {
...
// ═══════════════════════════════════════════════════════════════
// LEVEL 1: MARKET (MSA) DETAIL — "Atlanta, GA"
// ═══════════════════════════════════════════════════════════════
...
function MSAOverview() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 1, background: T.borderS, flex: 1, overflow: "auto" }}>
      ...
      {/* 3-COLUMN: Rent Chart | Key Metrics | Market Profile */}
...
// ═══════════════════════════════════════════════════════════════
// LEVEL 2: SUBMARKET DETAIL — "Midtown, Atlanta"
// ═══════════════════════════════════════════════════════════════
...
function SubmarketOverview() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 1, background: T.borderS, flex: 1, overflow: "auto" }}>
      ...
      {/* 3-COLUMN: Rent Chart | Key Metrics | Submarket Profile */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 1 }}>
        {/* Rent Chart */}
        <div style={{ background: T.panel, padding: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
      ...
      {/* PEER COMPARISON — Submarket vs Submarket (like REL VALUE inline) */}
      <div style={{ background: T.panel }}>
        ...
        {SUB_COMPS.map((c,i) => (
          <div key={i} style={{ display: "flex", background: c.isSub ? T.amber + "0A" : i % 2 === 0 ? T.panel : T.panelAlt, borderBottom: `1px solid ${T.borderS}`, borderLeft: c.isSub ? `2px solid ${T.amber}` : "2px solid transparent", cursor: "pointer" }}
            onMouseEnter={e => { if (!c.isSub) e.currentTarget.style.background = T.hover; }}
            onMouseLeave={e => { if (!c.isSub) e.currentTarget.style.background = i % 2 === 0 ? T.panel : T.panelAlt; }}>
...
// ═══════════════════════════════════════════════════════════════
// MAIN APP — Tabbed Navigation Between Levels
// ═══════════════════════════════════════════════════════════════
export default function MarketSubmarketViews() {
    ...
    <div style={{ background: T.bg, height: "100vh", display: "flex", flexDirection: "column", color: T.primary, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 22, padding: "0 10px", background: T.topBar, borderBottom: `1px solid ${T.borderS}`, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 8, color: T.secondary, ...mono }}>F4 MARKETS</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "0 10px", height: 28, background: T.header, borderBottom: `1px solid ${T.borderM}`, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 8, color: T.muted, ...mono }}>Click a level to see its Bloomberg Overview treatment</span>
      </div>
