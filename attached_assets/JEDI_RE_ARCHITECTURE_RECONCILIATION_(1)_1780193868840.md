...
| Market context F1 vs F8 | Same data, same framing — intent unclear | Both surfaces read `deal_market_intelligence`; both empty for 464 Bishop so not currently operator-visible. May be intentional dual-surfacing or accidental duplication. | Investigation needed before fix. Task drafted; see §5. |
| Backend double-mounts | Same handler, multiple mount points | No visible UI impact; potential confusion in logs, potential double-execution of middleware. | Cleanup task drafted; see §5. |
