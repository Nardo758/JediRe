...
function computeProgram(program) {
...
function computeGaps(inventory) {
...
//  SHARED ATOMS
// ─────────────────────────────────────────────────────────
function Tag({ label, color, size = "sm" }) {
  return (
...
function SecLabel({ mod, title, sub }) {
  return (
...
function NumInput({ value, onChange, min, max, step = 1, width = 60, suffix = "", accent }) {
  return (
...
function MiniLine({ data, dataKey, color }) {
  return (
...
// ─────────────────────────────────────────────────────────
function DemandMatrix({ inventory }) {
  return (
...
// ─────────────────────────────────────────────────────────
function ZoningPanel({ zoning, program, computed, onZoningChange }) {
...
// ─────────────────────────────────────────────────────────
function ProgramEditor({ program, computed, zoning, onProgramChange }) {
  const { totalSF, mixTotal, grossRevPA } = computed;
  const mixOk = Math.abs(mixTotal - 100) < 1;

  function setUnit(utKey, field, val) {
...
// ─────────────────────────────────────────────────────────
function PropertyDrillDown({ selectedType, onSelect }) {
...
// ─────────────────────────────────────────────────────────
function TrendDetail({ selectedType, onSelect }) {
  const ut = UT_META.find(u => u.key === selectedType);
  const data = TREND_DATA[selectedType];
  const metrics = [
    { key: "vac",  label: "Vacancy %",            color: C.red,    fmt: v => `${v}%`  },
    { key: "dom",  label: "Days on Market",        color: C.yellow, fmt: v => `${v}d`  },
    { key: "rent", label: "Avg Rent",              color: C.green,  fmt: v => `$${v}`  },
    { key: "conc", label: "Concessions (wks free)",color: C.studio, fmt: v => `${v}wk` },
  ];
...
// ─────────────────────────────────────────────────────────
function RentSFScatter({ program, filterUT, setFilterUT }) {
