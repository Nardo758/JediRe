...
export const OverviewSection: React.FC<OverviewSectionProps> = ({
  ...
  // Determine if this is a development deal (vs existing acquisition)
  ...
  useEffect(() => {
    const newMode = isDev ? 'development' : 'existing';
    console.log('[OverviewSection] Setting viewMode:', {
      dealId: deal?.id,
      dealName: deal?.name,
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Render only ONE overview based on viewMode */}
      {viewMode === 'existing' && (
        <ExistingOverview 
          deal={deal} 
          navigateToTab={navigateToTab} 
...
const DevOverview: React.FC<DevOverviewProps> = ({ deal, navigateToTab, financial, design3D, activeScenario, zoningProfile }) => {
  ...
  const { updateDevelopmentProgram } = useDealModule();
  ...
  // Update development program context when path or unit mix changes
  useEffect(() => {
    if (activePath && unitMix.length > 0) {
      ...
      console.log('[DevOverview] Updated development program:', {
  ...
  }, [selectedPathId, activePath?.id, activePath?.units, totalUnitCount, totalRevMo, avgSqft, avgRent]);
