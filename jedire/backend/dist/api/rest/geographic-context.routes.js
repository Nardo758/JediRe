...
router.post('/:id/geographic-context', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        ...
        logger_1.logger.info(`Geographic context set for deal ${dealId}`, { context });
        res.status(201).json({
            success: true,
            data: context,
        ...
        logger_1.logger.error('Error setting geographic context:', error);
        next(error);
    }
});
// GET /api/v1/deals/:id/geographic-context - Get deal's geographic hierarchy
router.get('/:id/geographic-context', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const { id: dealId } = req.params;
        ...
        logger_1.logger.error('Error fetching geographic context:', error);
        next(error);
    }
});
// PUT /api/v1/deals/:id/geographic-context - Update active scope
router.put('/:id/geographic-context', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const { id: dealId } = req.params;
        ...
        logger_1.logger.info(`Updated active scope for deal ${dealId} to ${active_scope}`);
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error updating geographic context:', error);
        next(error);
    }
});
// GET /api/v1/submarkets/lookup - Find submarket for coordinates
router.get('/submarkets/lookup', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        ...
        logger_1.logger.error('Error looking up submarket:', error);
        next(error);
    }
});
// GET /api/v1/msas/lookup - Find MSA for coordinates
router.get('/msas/lookup', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const { lat, lng } = req.query;
        if (!lat || !lng) {
            return res.status(400).json({
        ...
        logger_1.logger.error('Error looking up MSA:', error);
        next(error);
    }
});
exports.default = router;
//# sourceMappingURL=geographic-context.routes.js.map