...
router.get('/', async (req, res) => {
    ...
    `;
        ...
        logger_1.logger.error('Error fetching map configs:', error);
        res.status(500).json({
            success: false,
...
router.get('/default', async (req, res) => {
    try {
        const userId = req.user?.userId;
        const result = await (0, connection_1.query)('SELECT * FROM get_default_map_config($1)', [userId]);
        ...
        logger_1.logger.error('Error fetching default map config:', error);
        res.status(500).json({
            success: false,
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.query)(`SELECT * FROM map_configurations
       WHERE id = $1 AND (user_id = $2 OR is_public = true)`, [configId, userId]);
        if (result.rows.length === 0) {
        ...
        await (0, connection_1.query)('SELECT increment_map_config_views($1)', [configId]);
        res.json({
            success: true,
            data: result.rows[0]
        });
        ...
        logger_1.logger.error('Error fetching map config:', error);
        res.status(500).json({
            success: false,
...
router.post('/', async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { name, description, icon, config_type = 'custom', is_default = false, is_public = false, layer_config, map_center, map_zoom } = req.body;
        // Validation
        ...
        const result = await (0, connection_1.query)(`INSERT INTO map_configurations (
        user_id,
        name,
        description,
        icon,
      ...
      RETURNING *`, [
        ...
        logger_1.logger.info('Map configuration created', {
            userId,
        ...
        logger_1.logger.error('Error creating map config:', error);
        res.status(500).json({
            success: false,
...
router.put('/:id', async (req, res) => {
    try {
        const userId = req.user?.userId;
        const configId = req.params.id;
        const { name, description, icon, is_default, is_public, layer_config, map_center, map_zoom } = req.body;
        ...
        const result = await (0, connection_1.query)(`UPDATE map_configurations
       SET ${updates.join(', ')}
       ...
       RETURNING *`, values);
        ...
        logger_1.logger.info('Map configuration updated', { userId, configId });
        res.json({
            success: true,
            data: result.rows[0],
        ...
        logger_1.logger.error('Error updating map config:', error);
        res.status(500).json({
            success: false,
...
router.delete('/:id', async (req, res) => {
    try {
        const userId = req.user?.userId;
        const configId = req.params.id;
        const result = await (0, connection_1.query)('DELETE FROM map_configurations WHERE id = $1 AND user_id = $2 RETURNING id', [configId, userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'Map configuration not found or no permission'
        ...
        logger_1.logger.info('Map configuration deleted', { userId, configId });
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error deleting map config:', error);
        res.status(500).json({
            success: false,
...
router.post('/:id/clone', async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.query)('SELECT clone_map_config($1, $2, $3) as new_id', [configId, name, userId]);
        const newId = result.rows[0].new_id;
        // Fetch the new configuration
        const newConfig = await (0, connection_1.query)('SELECT * FROM map_configurations WHERE id = $1', [newId]);
        logger_1.logger.info('Map configuration cloned', {
            userId,
            originalId: configId,
            newId
        });
        ...
        logger_1.logger.error('Error cloning map config:', error);
        res.status(500).json({
            success: false,
...
router.post('/:id/set-default', async (req, res) => {
    try {
       ...
       RETURNING *`, [configId, userId]);
        ...
        logger_1.logger.info('Map configuration set as default', { userId, configId });
        res.json({
            success: true,
            data: result.rows[0],
        ...
        logger_1.logger.error('Error setting default map config:', error);
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=map-configs.routes.js.map