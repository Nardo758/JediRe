 ...
 */
router.get('/findings', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        ...
        if (!category || category === 'all' || category === 'news') {
            try {
                const newsResult = await (0, connection_1.query)(`SELECT 
            ne.id,
            ne.event_type,
            ne.event_category,
            ne.impact_severity,
        ...
        if (!category || category === 'all' || category === 'market') {
            try {
                ...
                const marketResult = await (0, connection_1.query)(`SELECT 
          d.id as deal_id,
          d.name as deal_name,
        ...
        if (!category || category === 'all' || category === 'insights') {
            try {
                const insightsResult = await (0, connection_1.query)(`SELECT 
          ar.id,
          ar.deal_id,
          d.name as deal_name,
          d.address,
        ...
        if (!category || category === 'all' || category === 'actions') {
            try {
                const actionsResult = await (0, connection_1.query)(`SELECT 
            d.id,
            d.name,
            d.state,
            d.updated_at
 ...
 */
router.get('/assets', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user?.userId;
