...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.query)(`INSERT INTO strategy_analyses 
       ...
       RETURNING *`, [
        ...
        logger_1.logger.info('Strategy analysis created:', {
            userId,
            dealId,
            strategySlug,
        ...
        logger_1.logger.error('Error creating strategy analysis:', error);
        res.status(500).json({
            success: false,
...
router.get('/:dealId', async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.query)(`SELECT * FROM strategy_analyses 
       WHERE deal_id = $1
       ORDER BY created_at DESC`, [dealId]);
        ...
        logger_1.logger.error('Error fetching strategy analyses:', error);
        res.status(500).json({
            success: false,
...
router.post('/compare', async (req, res) => {
    try {
       ...
       ORDER BY recommended DESC, risk_score ASC`, [dealId, ...strategySlugs]);
        ...
        const bestIRR = comparison.reduce((best, curr) => curr.irr > best.irr ? curr : best, comparison[0] || {});
        const lowestRisk = comparison.reduce((best, curr) => curr.risk_score < best.risk_score ? curr : best, comparison[0] || {});
        res.json({
            success: true,
            data: {
        ...
        logger_1.logger.error('Error comparing strategies:', error);
        res.status(500).json({
            success: false,
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const analysisCheck = await (0, connection_1.query)(`SELECT sa.id FROM strategy_analyses sa
       JOIN deals d ON sa.deal_id = d.id
       WHERE sa.id = $1 AND d.user_id = $2`, [id, userId]);
        ...
        values.push(id);
        const result = await (0, connection_1.query)(`UPDATE strategy_analyses 
       SET ${updates.join(', ')}
       WHERE id = $${paramIndex}
       RETURNING *`, values);
        logger_1.logger.info('Strategy analysis updated:', {
            userId,
            analysisId: id
        });
        res.json({
        ...
        logger_1.logger.error('Error updating strategy analysis:', error);
        res.status(500).json({
            success: false,
...
router.delete('/:id', async (req, res) => {
    try {
       ...
       WHERE sa.id = $1 AND d.user_id = $2`, [id, userId]);
        ...
        await (0, connection_1.query)('DELETE FROM strategy_analyses WHERE id = $1', [id]);
        logger_1.logger.info('Strategy analysis deleted:', {
            userId,
            analysisId: id
        });
        res.json({
        ...
        logger_1.logger.error('Error deleting strategy analysis:', error);
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=strategy-analyses.routes.js.map