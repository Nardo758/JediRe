...
function getRequestOrigin(req) {
    ...
    if (!host) {
        throw new errorHandler_1.AppError(500, 'Unable to determine request host for OAuth callback');
    }
    return `${protocol}://${host}`;
}
function getGmailCallbackConfig(req) {
...
function resolveGmailCallbackUrl(req) {
...
function summarizeClientId(clientId) {
    if (!clientId)
        return null;
...
function parseStatusCode(statusParam, detail) {
...
function parseOAuthErrorForDiagnostics(detail, statusCode) {
    if (!detail)
        return null;
...
router.get('/oauth-diagnostics', auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const authCallback = process.env.GOOGLE_AUTH_CALLBACK_URL
        ...
        logger_1.logger.error('Error generating Gmail OAuth diagnostics:', error);
        next(error);
    }
});
router.get('/auth-url', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const callbackUrl = resolveGmailCallbackUrl(req);
        const state = encodeState({ userId, callbackUrl });
        const authUrl = gmail_sync_service_1.gmailSyncService.getAuthUrl(state, callbackUrl);
        res.json({
            success: true,
            data: {
                authUrl,
...
router.get('/callback', async (req, res, next) => {
    try {
        ...
        if (!code || typeof code !== 'string') {
            throw new errorHandler_1.AppError(400, 'Authorization code required');
        }
        const parsedState = decodeState(typeof state === 'string' ? state : undefined);
        const userId = parsedState?.userId;
        const callbackUrl = parsedState?.callbackUrl;
        if (!userId) {
            throw new errorHandler_1.AppError(400, 'Invalid state parameter');
        }
        const tokens = await gmail_sync_service_1.gmailSyncService.exchangeCodeForTokens(code, callbackUrl);
        const existingResult = await (0, connection_1.query)('SELECT id, refresh_token FROM user_email_accounts WHERE user_id = $1 AND email_address = $2', [userId, tokens.email]);
        let accountId;
        if (existingResult.rows.length > 0) {
         ...
         WHERE id = $4`, [tokens.accessToken, refreshToken, tokens.expiresAt, accountId]);
            logger_1.logger.info(`Gmail account reconnected: ${tokens.email}`);
        }
        else {
            const result = await (0, connection_1.query)(`INSERT INTO user_email_accounts (
          user_id, provider, email_address, access_token, refresh_token, token_expires_at
        ) VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id`, [userId, 'google', tokens.email, tokens.accessToken, tokens.refreshToken, tokens.expiresAt]);
            accountId = result.rows[0].id;
            logger_1.logger.info(`New Gmail account connected: ${tokens.email}`);
        }
        if (tokens.grantedScopes.length > 0) {
            logger_1.logger.info(`Gmail OAuth scopes granted for ${tokens.email}: ${tokens.grantedScopes.join(', ')}`);
        }
        gmail_sync_service_1.gmailSyncService.syncEmails(accountId, 50).catch(error => {
        ...
        logger_1.logger.error('Gmail callback error:', error);
...
router.post('/connect', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const callbackUrl = resolveGmailCallbackUrl(req);
        const state = encodeState({ userId, callbackUrl });
        ...
        logger_1.logger.error('Error initiating Gmail connection:', error);
        next(error);
    }
});
router.get('/accounts', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user.userId;
       ...
       ORDER BY is_primary DESC, created_at DESC`, [userId]);
        ...
        logger_1.logger.error('Error fetching Gmail accounts:', error);
        next(error);
    }
});
router.get('/sync', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { accountId } = req.query;
        if (accountId) {
            const accountResult = await (0, connection_1.query)('SELECT id FROM user_email_accounts WHERE id = $1 AND user_id = $2', [accountId, userId]);
            if (accountResult.rows.length === 0) {
                throw new errorHandler_1.AppError(404, 'Account not found');
            }
            const result = await gmail_sync_service_1.gmailSyncService.syncEmails(accountId);
            res.json({
                success: true,
                data: {
                    accountId,
            ...
            const accountsResult = await (0, connection_1.query)('SELECT id FROM user_email_accounts WHERE user_id = $1 AND provider = $2 AND sync_enabled = true', [userId, 'google']);
            const accounts = accountsResult.rows;
            const syncResults = [];
            for (const account of accounts) {
                try {
                    const result = await gmail_sync_service_1.gmailSyncService.syncEmails(account.id);
                    syncResults.push({
                    ...
                    logger_1.logger.error(`Sync failed for account ${account.id}:`, error);
                    syncResults.push({
                        accountId: account.id,
                        success: false,
...
router.post('/sync/:accountId', auth_1.requireAuth, async (req, res, next) => {
    try {
        const { accountId } = req.params;
        const userId = req.user.userId;
        const accountResult = await (0, connection_1.query)('SELECT id FROM user_email_accounts WHERE id = $1 AND user_id = $2', [accountId, userId]);
        if (accountResult.rows.length === 0) {
            throw new errorHandler_1.AppError(404, 'Account not found or access denied');
        }
        const result = await gmail_sync_service_1.gmailSyncService.syncEmails(accountId);
        res.json({
            success: true,
...
router.delete('/disconnect/:accountId', auth_1.requireAuth, async (req, res, next) => {
    try {
        const { accountId } = req.params;
        const userId = req.user.userId;
        const accountResult = await (0, connection_1.query)('SELECT email_address FROM user_email_accounts WHERE id = $1 AND user_id = $2', [accountId, userId]);
        if (accountResult.rows.length === 0) {
            throw new errorHandler_1.AppError(404, 'Account not found or access denied');
        }
        const emailAddress = accountResult.rows[0].email_address;
        await (0, connection_1.query)('DELETE FROM user_email_accounts WHERE id = $1', [accountId]);
        logger_1.logger.info(`Gmail account disconnected: ${emailAddress}`);
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error disconnecting Gmail account:', error);
        next(error);
    }
});
router.patch('/accounts/:accountId', auth_1.requireAuth, async (req, res, next) => {
    try {
        const { accountId } = req.params;
        const userId = req.user.userId;
        ...
        await (0, connection_1.query)(`UPDATE user_email_accounts SET ${updates.join(', ')} WHERE id = $${paramCount}`, values);
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error updating account settings:', error);
        next(error);
    }
});
router.get('/emails', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user.userId;
    ...
    `;
        ...
        const result = await (0, connection_1.query)(queryStr, values);
        ...
        const countResult = await (0, connection_1.query)(countQuery, countValues);
...
router.get('/sync-logs', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user.userId;
    ...
    `;
        ...
        logger_1.logger.error('Error fetching sync logs:', error);
        next(error);
    }
});
