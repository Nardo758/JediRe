 ...
 */
router.get('/events', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        const { category, source_type, severity, limit = 50, offset = 0, include_private = 'true', } = req.query;
        let whereConditions = [];
        const params = [];
        let paramIndex = 1;
        // Filter by category
    ...
    `;
      ...
      ${whereClause}
    `;
        const countResult = await (0, connection_1.query)(countSql, params.slice(0, -2));
        res.json({
            success: true,
            data: result.rows,
            pagination: {
 ...
 */
router.get('/events/:id', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const result = await (0, connection_1.query)(`SELECT ne.*,
 ...
 */
router.get('/dashboard', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        const { trade_area_id, submarket_id } = req.query;
        // Get demand momentum (employment events)
        const demandResult = await (0, connection_1.query)(`SELECT 
        SUM(CASE 
      ...
      WHERE ne.event_category = 'employment'
        ...
        AND (ne.source_type = 'public' OR ne.source_user_id = $1)`, [userId]);
        ...
        const supplyResult = await (0, connection_1.query)(`SELECT 
      ...
      WHERE ne.event_category = 'development'
        ...
        const transactionsResult = await (0, connection_1.query)(`SELECT 
 ...
 */
router.get('/alerts', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user?.userId;
    ...
    `;
        const result = await (0, connection_1.query)(sql, params);
        // Get unread count
        const countResult = await (0, connection_1.query)('SELECT COUNT(*) as unread FROM news_alerts WHERE user_id = $1 AND is_read = FALSE', [userId]);
        res.json({
            success: true,
            data: result.rows,
 ...
 */
router.patch('/alerts/:id', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
 ...
 */
router.get('/network', auth_1.authMiddleware.requireAuth, async (req, res, next) => {
    try {
        const userId = req.user?.userId;
      ...
      LIMIT 20`, [userId]);
        ...
        const earlySignalResult = await (0, connection_1.query)(`SELECT AVG(early_signal_days) as avg_early_days
      FROM news_events ne
      WHERE ne.source_user_id = $1
        AND ne.early_signal_days IS NOT NULL`, [userId]);
        res.json({
            success: true,
            data: {
