...
exports.extractNewsFromEmail = extractNewsFromEmail;
exports.storeNewsEvent = storeNewsEvent;
exports.linkEmailToNews = linkEmailToNews;
exports.processEmailForNews = processEmailForNews;
 ...
 */
async function extractNewsFromEmail(emailId, subject, body, from, receivedAt) {
    try {
        const emailContent = `
Subject: ${subject}
From: ${from}
...
Return valid JSON only.`;
        ...
        if (extracted.confidence < 0.3) {
            logger_1.logger.debug('Email does not contain significant news', { emailId });
            return null;
        }
        return {
            ...extracted,
        ...
        logger_1.logger.error('Error extracting news from email:', error);
        return null;
    }
}
/**
 ...
 */
function assessSourceCredibility(from) {
 ...
 */
async function findMarketId(city, state) {
    if (!city && !state)
        return null;
    try {
        const result = await (0, connection_1.query)(`SELECT id FROM markets 
       ...
       LIMIT 1`, [city, state]);
        ...
        logger_1.logger.error('Error finding market:', error);
        return null;
    }
}
/**
 ...
 */
async function storeNewsEvent(newsEvent, emailId, userId) {
    try {
        // Geocode location
        let location = null;
        ...
        const result = await (0, connection_1.query)(`INSERT INTO news_items (
        market_id,
        title,
        summary,
        source,
        ...
        logger_1.logger.info('News event stored', {
            newsItemId,
            emailId,
        ...
        logger_1.logger.error('Error storing news event:', error);
        return null;
    }
}
/**
 ...
 */
async function linkEmailToNews(emailId, newsItemId) {
    try {
       ...
       WHERE id = $1`, [emailId, newsItemId]);
        logger_1.logger.debug('Email linked to news item', { emailId, newsItemId });
    }
    catch (error) {
        logger_1.logger.error('Error linking email to news:', error);
    }
}
/**
 * Main function: Extract and store news from email
 */
async function processEmailForNews(emailId, subject, body, from, receivedAt, userId) {
    try {
        logger_1.logger.info('Processing email for news extraction', {
            emailId,
            subject,
            from,
        });
        ...
        logger_1.logger.error('Error processing email for news:', error);
        return {
            success: false,
...
//# sourceMappingURL=email-news-extraction.service.js.map