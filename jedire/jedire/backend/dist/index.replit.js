...
exports.io = exports.pool = exports.app = void 0;
/**
...
const email_sync_scheduler_1 = require("./services/email-sync-scheduler");
...
app.get('/api/v1/supply/:market', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT * FROM supply_metrics 
       WHERE market = $1 
...
app.get('/api/v1/alerts', auth_1.requireAuth, async (req, res) => {
    try {
...
app.post('/api/v1/auth/login', async (req, res) => {
    try {
        ...
        if (email === 'demo@jedire.com' && password === 'demo123') {
            const result = await pool.query('SELECT id, email, full_name, role, subscription_tier, enabled_modules FROM users WHERE email = $1', [email]);
...
app.get('/api/v1/auth/me', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const result = await pool.query('SELECT id, email, full_name, role, subscription_tier, enabled_modules FROM users WHERE id = $1', [userId]);
        if (result.rows.length === 0) {
...
app.get('/api/v1/deals', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
...
// Get a single deal
app.get('/api/v1/deals/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const result = await client.query(`
      SELECT 
...
// Create a new deal
app.post('/api/v1/deals', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const { name, boundary, projectType, projectIntent, targetUnits, budget, timelineStart, timelineEnd, tier, deal_category, development_type, address, description } = req.body;
        if (!name || !boundary) {
...
// Update a deal
app.patch('/api/v1/deals/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const dealId = req.params.id;
        const updates = req.body;
        const dealCheck = await client.query('SELECT id FROM deals WHERE id = $1 AND user_id = $2 AND archived_at IS NULL', [dealId, req.user.userId]);
        ...
        values.push(dealId);
        const result = await client.query(`UPDATE deals SET ${setClauses.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values);
...
app.delete('/api/v1/deals/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const dealId = req.params.id;
        const result = await client.query('UPDATE deals SET archived_at = NOW(), updated_at = NOW() WHERE id = $1 AND user_id = $2 AND archived_at IS NULL RETURNING id', [dealId, req.user.userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Deal not found' });
        }
...
// Get deal modules
app.get('/api/v1/deals/:id/modules', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const dealId = req.params.id;
        const dealCheck = await client.query('SELECT id FROM deals WHERE id = $1 AND user_id = $2', [dealId, req.user.userId]);
        if (dealCheck.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Deal not found' });
        }
        const result = await client.query('SELECT * FROM deal_modules WHERE deal_id = $1', [dealId]);
...
app.get('/api/v1/deals/:id/activity', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const dealId = req.params.id;
        const limit = parseInt(req.query.limit) || 50;
        try {
            const result = await client.query(`SELECT id, deal_id, user_id, action_type, entity_type, entity_id, description, metadata, created_at
         FROM deal_activity
         WHERE deal_id = $1
         ORDER BY created_at DESC
         LIMIT $2`, [dealId, limit]);
...
// Get deal timeline
app.get('/api/v1/deals/:id/timeline', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const dealId = req.params.id;
        try {
            const result = await client.query(`SELECT id, deal_id, action_type, description, metadata, created_at
         FROM deal_activity
         WHERE deal_id = $1
         ORDER BY created_at ASC`, [dealId]);
            const now = new Date();
            const eventsByDate = {};
...
app.get('/api/v1/deals/:id/key-moments', auth_1.requireAuth, async (req, res) => {
    try {
        const client = req.dbClient || pool;
        const dealId = req.params.id;
        try {
         ...
         ORDER BY created_at DESC`, [dealId]);
            const momentTypeMap = {
...
let nextTaskId = 4;
app.get('/api/v1/tasks', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (dealId)
            filtered = filtered.filter(t => t.dealId === parseInt(dealId));
        if (category)
...
app.get('/api/v1/tasks/stats', auth_1.requireAuth, async (req, res) => {
    try {
        const now = new Date();
...
app.post('/api/v1/tasks', auth_1.requireAuth, async (req, res) => {
    try {
...
app.patch('/api/v1/tasks/:id', auth_1.requireAuth, async (req, res) => {
    try {
...
app.delete('/api/v1/tasks/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const idx = taskStore.findIndex(t => t.id === parseInt(req.params.id));
        if (idx === -1)
            return res.status(404).json({ success: false, error: 'Task not found' });
...
// ============================================
app.get('/api/v1/inbox', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const limit = parseInt(req.query.limit) || 50;
        ...
        params.push(limit);
        params.push(offset);
        const result = await pool.query(`SELECT e.*, d.name as deal_name,
...
app.get('/api/v1/inbox/stats', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
       ...
       FROM emails WHERE user_id = $1 AND NOT is_archived`, [userId]);
...
app.get('/api/v1/inbox/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const emailId = parseInt(req.params.id);
        const result = await pool.query(`SELECT e.*, d.name as deal_name FROM emails e LEFT JOIN deals d ON e.deal_id = d.id WHERE e.id = $1 AND e.user_id = $2`, [emailId, userId]);
        if (result.rows.length === 0) {
...
app.patch('/api/v1/inbox/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const emailId = parseInt(req.params.id);
        const updates = req.body;
        ...
        await pool.query(`UPDATE emails SET ${setClauses.join(', ')} WHERE id = $${paramIndex} AND user_id = $${paramIndex + 1}`, params);
...
// ============================================
const news_routes_1 = __importDefault(require("./api/rest/news.routes"));
const trade_areas_routes_1 = __importDefault(require("./api/rest/trade-areas.routes"));
const geographic_context_routes_1 = __importDefault(require("./api/rest/geographic-context.routes"));
const isochrone_routes_1 = __importDefault(require("./api/rest/isochrone.routes"));
const traffic_ai_routes_1 = __importDefault(require("./api/rest/traffic-ai.routes"));
const map_configs_routes_1 = __importDefault(require("./api/rest/map-configs.routes"));
const grid_routes_1 = __importDefault(require("./api/rest/grid.routes"));
const modules_routes_1 = __importDefault(require("./api/rest/modules.routes"));
const financial_models_routes_1 = __importDefault(require("./api/rest/financial-models.routes"));
const strategy_analyses_routes_1 = __importDefault(require("./api/rest/strategy-analyses.routes"));
const dd_checklists_routes_1 = __importDefault(require("./api/rest/dd-checklists.routes"));
const dashboard_routes_1 = __importDefault(require("./api/rest/dashboard.routes"));
const gmail_routes_1 = __importDefault(require("./api/rest/gmail.routes"));
app.use('/api/v1/dashboard', dashboard_routes_1.default);
app.use('/api/v1/gmail', gmail_routes_1.default);
app.use('/api/v1/news', news_routes_1.default);
app.use('/api/v1/trade-areas', trade_areas_routes_1.default);
app.use('/api/v1/isochrone', isochrone_routes_1.default);
app.use('/api/v1/traffic-ai', traffic_ai_routes_1.default);
app.use('/api/v1', geographic_context_routes_1.default);
app.use('/api/v1/deals', geographic_context_routes_1.default);
app.use('/api/v1/map-configs', map_configs_routes_1.default);
app.use('/api/v1/grid', grid_routes_1.default);
app.use('/api/v1/modules', auth_1.requireAuth, modules_routes_1.default);
app.use('/api/v1/financial-models', financial_models_routes_1.default);
app.use('/api/v1/strategy-analyses', strategy_analyses_routes_1.default);
app.use('/api/v1/dd-checklists', dd_checklists_routes_1.default);
// ============================================
...
app.post('/api/v1/geocode', async (req, res) => {
    try {
        ...
        const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}&limit=1&addressdetails=1`, { headers: { 'User-Agent': 'JediRE/1.0 (contact@jedire.com)' } });
...
app.get('/api/v1/zoning/districts/:municipality', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT id, district_code, district_name, description, permitted_uses, 
              max_building_height_ft, max_units_per_acre, max_far
       FROM zoning_districts 
       WHERE LOWER(municipality) = LOWER($1) AND LOWER(state) = LOWER($2)
       ORDER BY district_code`, [municipality, state]);
        res.json({ success: true, data: result.rows });
    }
    catch (error) {
...
app.post('/api/v1/analyze', async (req, res) => {
    ...
    `;
        ...
        if (maxUnits >= 4)
            ...
            score += 10;
        if (district.max_far && district.max_far >= 1.0)
            score += 15;
        else if (district.max_far && district.max_far >= 0.5)
            score += 5;
        if (district.max_building_height_ft && district.max_building_height_ft >= 40)
            score += 10;
        if (lot_size_sqft >= 10000)
            score += 10;
        else if (lot_size_sqft >= 7000)
            score += 5;
...
// ============================================
app.get('/api/v1/deals/:id/lease-analysis', auth_1.requireAuth, async (req, res) => {
    try {
        const dealId = req.params.id;
        const client = req.dbClient || pool;
        const dealCheck = await client.query('SELECT id FROM deals WHERE id = $1 AND user_id = $2', [dealId, req.user.userId]);
    ...
    `, [dealId]);
        ...
        const expiringNext30 = properties.filter((p) => p.lease_expiration_date &&
            new Date(p.lease_expiration_date) <= next30Days &&
            new Date(p.lease_expiration_date) >= now).length;
        const expiringNext90 = properties.filter((p) => p.lease_expiration_date &&
        ...
        const belowMarketUnits = properties.filter((p) => p.current_lease_amount && p.rent && p.current_lease_amount < p.rent);
        const totalRentGap = belowMarketUnits.reduce((sum, p) => sum + (p.rent - p.current_lease_amount), 0);
...
app.get('/api/v1/microsoft/status', auth_1.requireAuth, async (req, res) => {
    const configured = !!(microsoftConfig.clientId && microsoftConfig.clientSecret);
    let connected = false;
    try {
        const client = req.dbClient || pool;
        const result = await client.query('SELECT id FROM microsoft_accounts WHERE user_id = $1 AND is_active = true LIMIT 1', [req.user.userId]);
...
app.get('/api/v1/microsoft/auth/url', auth_1.requireAuth, (req, res) => {
...
app.get('/api/v1/microsoft/auth/callback', async (req, res) => {
    ...
    try {
        ...
        const tokenResponse = await axios.post(`https://login.microsoftonline.com/${microsoftConfig.tenantId}/oauth2/v2.0/token`, new URLSearchParams({
            client_id: microsoftConfig.clientId,
...
httpServer.listen(Number(PORT), '0.0.0.0', () => {
    ...
    console.log('='.repeat(60));
    try {
        email_sync_scheduler_1.emailSyncScheduler.start(15);
...
process.on('SIGTERM', async () => {
    ...
    try {
        email_sync_scheduler_1.emailSyncScheduler.stop();
...
//# sourceMappingURL=index.replit.js.map