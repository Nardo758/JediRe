...
async function main() {
  ...
  const propertiesWithOM = new Set<string>();
  const propertiesWithOperating = new Set<string>();
  const propertiesWithBoth = new Set<string>();
  ...
  for (const [name, data] of sortedClusters) {
    ...
    if (hasOM) propertiesWithOM.add(name);
