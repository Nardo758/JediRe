...
describe("plausibility scoring", () => {
  ...
  it("returns per-variable decomposition that sums to d\xB2", async () => {
    const input = {
      assumptions: {
        rent_growth: 0.04,
    ...
    const result = await scorePlausibility(input);
    const contribSum = Object.values(result.perVariable).reduce((s, v) => s + v.contribution, 0);
  ...
  it("generates warnings for out-of-range assumptions", async () => {
    const input = {
      assumptions: {
        rent_growth: 0.12,
        // 12% — way above 8% max feasible
        exit_cap_rate: 0.03
        // 3% — below 4% min feasible
      }
    };
    const result = await scorePlausibility(input);
