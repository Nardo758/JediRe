...
export const dealLeaseTransactions = pgTable('deal_lease_transactions', {
  ...
  leaseStatus: varchar('lease_status', { length: 30 }),
...
export const dealBalanceSheets = pgTable('deal_balance_sheets', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  dealId: uuid('deal_id').notNull(),
  reportMonth: date('report_month').notNull(),
  totalAssets: numeric('total_assets', { precision: 14, scale: 2 }),
  cash: numeric('cash', { precision: 14, scale: 2 }),
  accountsReceivable: numeric('accounts_receivable', { precision: 12, scale: 2 }),
  prepaidExpenses: numeric('prepaid_expenses', { precision: 12, scale: 2 }),
  realEstateAssets: numeric('real_estate_assets', { precision: 14, scale: 2 }),
  ...
  mortgageBalance: numeric('mortgage_balance', { precision: 14, scale: 2 }),
  accountsPayable: numeric('accounts_payable', { precision: 12, scale: 2 }),
  accruedExpenses: numeric('accrued_expenses', { precision: 12, scale: 2 }),
  securityDeposits: numeric('security_deposits', { precision: 12, scale: 2 }),
  otherLiabilities: numeric('other_liabilities', { precision: 12, scale: 2 }),
  totalEquity: numeric('total_equity', { precision: 14, scale: 2 }),
  partnerCapital: numeric('partner_capital', { precision: 14, scale: 2 }),
  retainedEarnings: numeric('retained_earnings', { precision: 14, scale: 2 }),
  reserves: numeric('reserves', { precision: 12, scale: 2 }),
  escrows: numeric('escrows', { precision: 12, scale: 2 }),
  taxEscrow: numeric('tax_escrow', { precision: 12, scale: 2 }),
  insuranceEscrow: numeric('insurance_escrow', { precision: 12, scale: 2 }),
  replacementReserve: numeric('replacement_reserve', { precision: 12, scale: 2 }),
  sourceType: varchar('source_type', { length: 30 }),
  sourceRef: varchar('source_ref', { length: 500 }),
  sourceDate: date('source_date'),
  notes: text('notes'),
  ...
  dealIdx: index('idx_balance_deal').on(table.dealId),
  monthIdx: index('idx_balance_month').on(table.reportMonth),
  dealMonthIdx: uniqueIndex('idx_balance_deal_month').on(table.dealId, table.reportMonth),
}));

export const dealCapexItems = pgTable('deal_capex_items', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  dealId: uuid('deal_id').notNull(),
  ...
  dealIdx: index('idx_capex_deal').on(table.dealId),
  categoryIdx: index('idx_capex_category').on(table.category),
  statusIdx: index('idx_capex_status').on(table.status),
}));

export const dealDebtSchedule = pgTable('deal_debt_schedule', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  dealId: uuid('deal_id').notNull(),
  ...
  spread: numeric('spread', { precision: 5, scale: 4 }),
  indexRate: varchar('index_rate', { length: 30 }),
  maturityDate: date('maturity_date'),
  originationDate: date('origination_date'),
  amortizationYears: integer('amortization_years'),
  ioPeriodMonths: integer('io_period_months'),
  monthlyPayment: numeric('monthly_payment', { precision: 12, scale: 2 }),
  monthlyPrincipal: numeric('monthly_principal', { precision: 12, scale: 2 }),
  monthlyInterest: numeric('monthly_interest', { precision: 12, scale: 2 }),
  annualDebtService: numeric('annual_debt_service', { precision: 14, scale: 2 }),
  ltv: numeric('ltv', { precision: 5, scale: 4 }),
  dscr: numeric('dscr', { precision: 5, scale: 2 }),
  debtYield: numeric('debt_yield', { precision: 5, scale: 4 }),
  prepaymentType: varchar('prepayment_type', { length: 50 }),
  prepaymentExpiry: date('prepayment_expiry'),
  ...
  dealIdx: index('idx_debt_deal').on(table.dealId),
  lenderIdx: index('idx_debt_lender').on(table.lender),
  maturityIdx: index('idx_debt_maturity').on(table.maturityDate),
  activeIdx: index('idx_debt_active').on(table.isActive),
}));
...
export type DealBalanceSheet = typeof dealBalanceSheets.$inferSelect;
export type NewDealBalanceSheet = typeof dealBalanceSheets.$inferInsert;
export type DealCapexItem = typeof dealCapexItems.$inferSelect;
export type NewDealCapexItem = typeof dealCapexItems.$inferInsert;
export type DealDebtSchedule = typeof dealDebtSchedule.$inferSelect;
export type NewDealDebtSchedule = typeof dealDebtSchedule.$inferInsert;
