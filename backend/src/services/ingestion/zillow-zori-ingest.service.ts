/**
 * Zillow ZORI (Zillow Observed Rent Index) Ingestion Service
 * Processes monthly rent index data for ZIP codes in Florida
...
/**
 * Ingest Zillow ZORI CSV file into metric_time_series
 */
export async function ingestZillowZORI(filePath: string): Promise<IngestionResult> {
  ...
  try {
    ...
    logger.info(`Parsed ${rows.length} rows from ZORI CSV`);
    ...
    for (const row of floridaZips) {
      try {
        ...
        for (const dateStr of dateColumns) {
          ...
          // Parse date from column header (e.g., "2017-01-31")
    ...
    logger.info('Computing Year-over-Year rent growth rates...');
    const yoyResult = await computeZORIYoYGrowth();
    result.yoyRowsInserted = yoyResult;

    result.endTime = new Date();
    const duration = result.endTime.getTime() - result.startTime.getTime();

    logger.info(`ZORI ingestion complete in ${duration}ms:`, {
    ...
    logger.error('ZORI ingestion failed:', error);
    throw error;
  }
}

/**
 * Compute Year-over-Year rent growth rates from ZORI base values
 */
async function computeZORIYoYGrowth(): Promise<number> {
  let rowsInserted = 0;

  try {
    // Get all distinct ZIP codes with ZORI data
    const zipsResult = await query(
      `
      SELECT DISTINCT geography_id FROM metric_time_series
      WHERE metric_id = 'F_RENT_INDEX' AND geography_type = 'zip'
      `
    );
    ...
    for (const zip of zips) {
      ...
      const valuesResult = await query(
        ...
        WHERE metric_id = 'F_RENT_INDEX' AND geography_id = $1 AND geography_type = 'zip'
