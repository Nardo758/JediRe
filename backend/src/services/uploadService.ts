...
): ProcessResult & { _records: NewDealMonthlyActual[] } {
  ...
  await db
    ...
    .onConflictDoUpdate({
      ...
      set: {
        ...
        lossToLease: sql`EXCLUDED.loss_to_lease`,
        ...
        utilityReimbursement: sql`EXCLUDED.utility_reimbursement`,
        lateFees: sql`EXCLUDED.late_fees`,
        miscIncome: sql`EXCLUDED.misc_income`,
        effectiveGrossIncome: sql`EXCLUDED.effective_gross_income`,
        payroll: sql`EXCLUDED.payroll`,
        repairsMaintenance: sql`EXCLUDED.repairs_maintenance`,
        turnoverCosts: sql`EXCLUDED.turnover_costs`,
        marketing: sql`EXCLUDED.marketing`,
        adminGeneral: sql`EXCLUDED.admin_general`,
        managementFee: sql`EXCLUDED.management_fee`,
        managementFeePct: sql`EXCLUDED.management_fee_pct`,
        utilities: sql`EXCLUDED.utilities`,
        contractServices: sql`EXCLUDED.contract_services`,
        propertyTax: sql`EXCLUDED.property_tax`,
        insurance: sql`EXCLUDED.insurance`,
        hoaCondoFees: sql`EXCLUDED.hoa_condo_fees`,
        totalOpex: sql`EXCLUDED.total_opex`,
        noi: sql`EXCLUDED.noi`,
        debtService: sql`EXCLUDED.debt_service`,
        debtServiceInterest: sql`EXCLUDED.debt_service_interest`,
        capex: sql`EXCLUDED.capex`,
        capexReserves: sql`EXCLUDED.capex_reserves`,
        ...
        leaseTradeOut: sql`EXCLUDED.lease_trade_out`,
        avgDaysToLease: sql`EXCLUDED.avg_days_to_lease`,
        adr: sql`EXCLUDED.adr`,
        revpar: sql`EXCLUDED.revpar`,
        strOccupancy: sql`EXCLUDED.str_occupancy`,
