...
export async function getCompSet(pool: Pool, dealId: string, tradeAreaId?: string): Promise<CompProperty[]> {
  ...
  const compIds = props.map((p: Record<string, unknown>) => p.id);
  const unitsQuery = `
  ...
  return props.map((prop: Record<string, unknown>) => {
    ...
    for (const ut of UNIT_TYPES) {
      const row = unitRows.find((r: Record<string, unknown>) => r.comp_id === prop.id && r.unit_type === ut);
      units[ut] = row
        ? {
    ...
    return {
      ...
      cls: prop.class as string,
      built: prop.built_year as number,
      total: prop.total_units as number,
      sourceUrl: prop.source_url as string | undefined,
      units: units as Record<UnitType, UnitData>,
    };
  });
}

export async function getTrends(pool: Pool, tradeAreaId: string): Promise<Record<string, TrendPoint[]>> {
  const query = `
  ...
  for (const ut of UNIT_TYPES) {
    result[ut] = rows
      .filter((r: Record<string, unknown>) => r.unit_type === ut)
      .slice(-12)
      .map((r: Record<string, unknown>) => ({
        mo: r.month_label as string,
...
export function computeDemandScores(compSet: CompProperty[]): DemandScore[] {
  return UNIT_TYPES.map(ut => {
    ...
    const avg = (fn: (c: CompProperty) => number) => active.reduce((s, c) => s + fn(c), 0) / active.length;
...
export async function saveProgram(
  ...
  userId: string,
  program: UnitProgram
): Promise<{ dealId: string; totalNetSf: number; grossRevPA: number; mixTotal: number }> {
...
export async function getProgram(pool: Pool, dealId: string): Promise<UnitProgram | null> {
  const query = `
  ...
  return {
    ...
    units: row.unit_config as Record<string, { mix: number; sf: number; rent: number }>,
    totalNetSf: row.total_net_sf as number,
    grossRevPA: Number(row.gross_rev_pa),
  };
}

export async function getZoningForUnitMix(pool: Pool, dealId: string): Promise<ZoningForUnitMix | null> {
  const query = `
  ...
  const row = rows[0];
  const maxUnits =
    row.max_density_per_acre && row.lot_area_sf
      ? Math.floor(row.max_density_per_acre * row.lot_area_sf / 43560)
      : null;
  const maxNetSf =
    row.applied_far && row.lot_area_sf
