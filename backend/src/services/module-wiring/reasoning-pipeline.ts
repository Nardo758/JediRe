...
export const REASONING_STAGES: readonly ReasoningStage[] = [
  ...
  },
  {
    ...
    description:     'M11 debt sizing (triple constraint F40), SOFR rate environment. M11 must have published to DataFlowRouter — loanAmount cannot be sized without proforma NOI being resolved first.',
    dependsOn:       ['proforma'],
    requiredModules: ['M11'] as unknown as ModuleId[],
    optionalModules: [],
  ...
  },
  {
    ...
    ordinal:         9,
    name:            'Exit Analysis',
    description:     'M12 exit cap rate, reversion value, optimal hold year. M12 must have published to DataFlowRouter — exit cap cannot be derived before market comp set and rate environment are known.',
    dependsOn:       ['market', 'capital_structure'],
    requiredModules: ['M12'] as unknown as ModuleId[],
    optionalModules: [],
    primaryAgentTool: 'fetch_comp_set',
  },
  {
