...
export class ZoningTriangulationService {
  ...
  async recordOutcome(input: {
    ...
    const municipalityData = original.reconciled_zoning_code
      ? await this.getMunicipalityFromTriangulation(original)
      : null;
    if (municipalityData) {
      await this.recalibrate(municipalityData.municipality, municipalityData.state);
    }
  ...
  async recalibrate(municipality: string, state: string): Promise<void> {
    ...
    if (outcomes.rows.length < 3) return;
  ...
  private reconcileCode(
    ...
    if (allMatch) {
      return {
        ...
        agreement: true,
        confidence: 0.60 + (codes.length * 0.13),
      };
    }
  ...
  private reconcileMath(
    ...
    const reconcileField = (
      ...
      const tolerance = Math.max(Math.abs(numMunicode) * 0.05, 1);
