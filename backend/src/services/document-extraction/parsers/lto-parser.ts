...
const LTO_HEADER_PATTERNS = [/unit|apt/i, /rent/i, /lease|new|renewal/i, /effective|eff/i, /market|mkt|asking/i, /prior|previous|old/i, /change|gain|loss|variance/i];
...
export function parseLTO(buffer: Buffer, filename: string): ExtractionResult {
  ...
  try {
    ...
    const { headers, rows } = smartParseSheet(sheet, LTO_HEADER_PATTERNS, 2);

    if (rows.length === 0) {
      return { documentType: 'T30_LTO', success: false, error: 'No data rows found (checked up to 20 title rows)', data: null, summary: {}, warnings };
    }
    ...
    if (records.length === 0) {
      return {
        ...
        error: 'No lease trade-out records extracted — columns found but no valid unit rows',
        data: null, summary: {}, warnings,
      };
    }

    if (records.every(r => (r.leaseRent == null || r.leaseRent === 0) && (r.effectiveRent == null || r.effectiveRent === 0) && (r.priorRent == null || r.priorRent === 0))) {
      return {
        documentType: 'T30_LTO', success: false,
        error: 'All rent values are null or zero — likely header detection failure',
        data: null, summary: {}, warnings,
      };
    }
