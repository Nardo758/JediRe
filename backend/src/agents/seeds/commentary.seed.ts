  ...
  try {
    await query(
      `UPDATE prompt_versions SET active = false
       WHERE agent_id = 'commentary' AND active = true AND id != 'commentary-v2'`
    );
    ...
    await query(
      `INSERT INTO prompt_versions
       ...
       VALUES
         ('commentary-v2', 'commentary', '2.0.0', $1, $2, true, NOW(), 'system')
    ...
    logger.info('Commentary Agent prompt seeded: commentary-v2 (active)');
  } catch (err) {
