...
export const supplyOnDealCreated = inngest.createFunction(
  ...
  async ({ event, step }): Promise<{ runId: string; confidence_score: number }> => {
    ...
    await step.run('credit-debit', async () => {
      ...
      await creditService.debitAgentRun(userId, 'supply');
      return { debited: true };
    });
      ...
      if (useNew || runShadow) {
        try {
          ...
          if (link?.propertyId) {
            ...
            newPropertyType = (propRes.rows[0]?.property_type as string | null) ?? null;
          }
          ...
          logger.warn('R-006 supply_agent new path failed; falling back to old path', { err, dealId });
        }
      }
