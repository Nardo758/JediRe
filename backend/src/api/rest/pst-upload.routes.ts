...
async function processPstAsync(jobId: string, uploadId: string, buffer: Buffer, userId: string): Promise<void> {
  ...
  try {
    ...
    try {
      await backflowPstToEmails(uploadId, userId);
      logger.info(`PST backflow complete for upload ${uploadId}`);
    } catch (bfErr: any) {
      logger.warn(`PST backflow failed (non-fatal): ${bfErr.message}`);
    }
...
async function backflowPstToEmails(uploadId: string, userId: string): Promise<void> {
