 ...
 *
 * Exercises the express router with `qwenService` mocked, so the suite runs
 * without a real HF_TOKEN or network access.
 */
...
vi.mock('../../../services/qwen.service', () => ({
  qwenService: {
    isEnabled: vi.fn(() => true),
    imageToTerrain: vi.fn(),
    analyzeDesignCompliance: vi.fn(),
    analyzeSiteFromAerial: vi.fn(),
    predictOwnerDisposition: vi.fn(),
    autoTagPhotos: vi.fn(),
...
describe('Qwen API Routes', () => {
  ...
  describe('GET /status', () => {
    it('should return AI service status when enabled', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
    ...
    it('should return disabled status when HF_TOKEN not configured', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(false);
  ...
  describe('POST /image-to-terrain', () => {
    ...
    it('should return terrain data on success', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
      (qwenService.imageToTerrain as Mock).mockResolvedValue({
  ...
  describe('POST /analyze-compliance', () => {
    ...
    it('should return compliance report on success', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
      (qwenService.analyzeDesignCompliance as Mock).mockResolvedValue({
        violations: [],
        compliant: true,
        confidence: 0.87,
  ...
  describe('POST /analyze-aerial', () => {
    ...
    it('should return site context on success', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
      (qwenService.analyzeSiteFromAerial as Mock).mockResolvedValue({
        adjacentParcels: [],
  ...
  describe('POST /owner-disposition', () => {
    ...
    it('should return disposition score on success', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
      (qwenService.predictOwnerDisposition as Mock).mockResolvedValue({
        score: 72,
  ...
  describe('POST /auto-tag-photos', () => {
    ...
    it('should return photo tags on success', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
      (qwenService.autoTagPhotos as Mock).mockResolvedValue([
        {
          photoId: 'photo-1',
  ...
  describe('POST /estimate-progress', () => {
    ...
    it('should return progress estimate on success', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
      (qwenService.estimateProgress as Mock).mockResolvedValue({
        section: 'floor-3',
        percentComplete: 65,
        confidence: 0.82,
  ...
  describe('POST /negotiation-strategy', () => {
    ...
    it('should return strategy on success', async () => {
      (qwenService.isEnabled as Mock).mockReturnValue(true);
      (qwenService.generateNegotiationStrategy as Mock).mockResolvedValue({
        approach: 'sequential',
        prioritizedTargets: [],
        timeline: '6-9 months',
        risks: [],
