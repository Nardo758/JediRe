 ...
 *
 * Honest-absence invariant: if a quote is missing, stale, or the curve is
 * unavailable, the response carries null with a reason — never fabricates.
 */
...
const compareSchema = z.object({
  deal: z.object({
    ...
    noiY1: z.number().positive(),
    targetLtv: z.number().min(0).max(1),
    preferredTerm: z.number().int().positive().optional(),
...
function serializeQuote(quote: LoanQuote) {
  return {
    ...
    indexBasis: quote.indexBasis,
    rateType: quote.rateType,
    spreadMatrix: quote.spreadMatrix,
    adjustments: quote.adjustments,
    prepayStructure: quote.prepayStructure,
    brokerClaims: quote.brokerClaims,
    notes: quote.notes,
...
/**
 * GET /api/v1/orgs/:orgId/quotes
 * List all quotes for an org. Optional query filters: ?lender=&program=
 */
router.get(
  '/',
  requireAuth,
  requireOrgRoleForOrg('viewer'),
  async (req: OrgAuthenticatedRequest, res: Response) => {
    try {
      const orgId = req.params.orgId;
      const { lender, program } = req.query;

      const quotes = await loanQuoteStore.list(orgId, {
...
/**
 * POST /api/v1/orgs/:orgId/quotes
 * Create a new quote for the org.
 */
router.post(
  '/',
  requireAuth,
  requireOrgRoleForOrg('analyst'),
  async (req: OrgAuthenticatedRequest, res: Response) => {
    try {
      const orgId = req.params.orgId;
      const parsed = createQuoteSchema.parse(req.body);

      const quote = await loanQuoteStore.create({
        orgId,
        lender: parsed.lender,
        program: parsed.program,
        quoteDate: parsed.quoteDate,
        expires: parsed.expires,
        indexBasis: parsed.indexBasis,
        rateType: parsed.rateType,
        spreadMatrix: parsed.spreadMatrix as LoanQuote['spreadMatrix'],
        adjustments: parsed.adjustments as LoanQuote['adjustments'],
        prepayStructure: parsed.prepayStructure as LoanQuote['prepayStructure'],
        brokerClaims: parsed.brokerClaims as LoanQuote['brokerClaims'],
        notes: parsed.notes,
      });
...
/**
 * GET /api/v1/orgs/:orgId/quotes/:id
 * Read a single quote. Honest-absence: 404 with reason if missing or wrong org.
 */
router.get(
  '/:id',
  requireAuth,
  ...
  async (req: OrgAuthenticatedRequest, res: Response) => {
    try {
      ...
      const quote = await loanQuoteStore.read(id, orgId);
      if (!quote) {
...
/**
 * PATCH /api/v1/orgs/:orgId/quotes/:id
 * Update a quote. Analyst+ required.
 */
router.patch(
  '/:id',
  requireAuth,
  ...
  async (req: OrgAuthenticatedRequest, res: Response) => {
    try {
      ...
      const quote = await loanQuoteStore.update(id, orgId, parsed as Partial<Omit<LoanQuote, 'id' | 'orgId' | 'createdAt'>>);
...
/**
 * DELETE /api/v1/orgs/:orgId/quotes/:id
 * Delete a quote. Analyst+ required.
 */
router.delete(
  '/:id',
  requireAuth,
  ...
  async (req: OrgAuthenticatedRequest, res: Response) => {
    try {
      ...
      const deleted = await loanQuoteStore.delete(id, orgId);
      if (!deleted) {
        return res.status(404).json({ success: false, error: 'Quote not found' });
      }
...
/**
 * POST /api/v1/orgs/:orgId/quotes/compare
 * Compare quotes for a deal. Uses the existing compareQuotes engine.
 *
 * Body: { deal: { purchasePrice, noiY1, targetLtv, preferredTerm?, preferredProgram? }, objective, quoteIds? }
 *
 * If quoteIds is omitted, all active (non-stale) quotes for the org are used.
 * If quoteIds is provided, only those quotes are fetched and compared.
 *
 * Honest-absence: stale quotes are flagged, failed quotes carry a reason.
 */
router.post(
  '/compare',
  requireAuth,
  ...
  async (req: OrgAuthenticatedRequest, res: Response) => {
    try {
      ...
      // Fetch quotes to compare
      let quotes: LoanQuote[];
      if (parsed.quoteIds && parsed.quoteIds.length > 0) {
        const fetched = await Promise.all(
          parsed.quoteIds.map((id) => loanQuoteStore.read(id, orgId))
        );
        quotes = fetched.filter((q): q is LoanQuote => q !== null);
        if (quotes.length === 0) {
          return res.status(404).json({
            success: false,
            error: 'None of the requested quote IDs were found for this organization',
          });
        }
      } else {
      ...
      res.json({
        success: true,
        data: {
          rankedQuotes: result.rankedQuotes.map((rq) => ({
            rank: rq.rank,
            rankScore: rq.rankScore,
            quote: serializeQuote(rq.quote),
            pricing: rq.pricing,
          })),
          staleQuotes: result.staleQuotes.map(serializeQuote),
          failedQuotes: result.failedQuotes.map((fq) => ({
            quote: serializeQuote(fq.quote),
            reason: fq.reason,
          })),
...
/**
 * GET /api/v1/orgs/:orgId/quotes/stale
 * List stale (expired) quotes for the org. Viewer+.
 */
router.get(
  '/stale',
  requireAuth,
  ...
  async (req: OrgAuthenticatedRequest, res: Response) => {
    try {
      const orgId = req.params.orgId;
      const quotes = await loanQuoteStore.findStale(orgId);
      res.json({ success: true, count: quotes.length, data: quotes.map(serializeQuote) });
    } catch (error: any) {
