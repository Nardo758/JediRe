...
export function createMarketIntelligenceRoutes(pool: Pool) {
  ...
  router.get('/properties/unified', async (req, res) => {
    try {
  ...
  router.post('/properties/unified/refresh', requireAuth, async (req, res) => {
    try {
      await pool.query('REFRESH MATERIALIZED VIEW v_unified_properties');
