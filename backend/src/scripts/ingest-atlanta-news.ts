  ...
  for (const { provider, queries } of providerMap) {
    for (const q of queries) {
      try {
        ...
        result.errors.push(`${provider.config.id}/"${q}": ${err.message}`);
      }
    }
  }

  console.log(`[Atlanta News] Ingested ${result.inserted} articles, skipped ${result.skipped}, errors: ${result.errors.length}`);
  return result;
}
